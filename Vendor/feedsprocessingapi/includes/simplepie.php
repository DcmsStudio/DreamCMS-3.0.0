...
class SimplePie
{
	...
	{
		$this->raw_data = trim((string)(string) $data);
	}
...
function embed_quicktime(type, bgcolor, width, height, link, placeholder, loop) {
	if (placeholder != '') {
		document.writeln('<embed type="'+type+'" style="cursor:hand; cursor:pointer;" href="'+link+'" src="'+placeholder+'" width="'+width+'" height="'+height+'" autbtay="false" target="myself" controller="false" loop="'+loop+'" scale="aspect" bgcolor="'+bgcolor+'" pluginspage="http://www.apple.com/quicktime/download/"></embed>');
	}
	else {
		document.writeln('<embed type="'+type+'" style="cursor:hand; cursor:pointer;" src="'+link+'" width="'+width+'" height="'+height+'" autbtay="false" target="myself" controller="true" loop="'+loop+'" scale="aspect" bgcolor="'+bgcolor+'" pluginspage="http://www.apple.com/quicktime/download/"></embed>');
	}
}
	...
	function embed($options = '', $native = false)
	{
		...
		else
		{
			...
			foreach($options as $option)
			{
				...
				{
					$opt[0] = trim((string)$opt[0]);
					$opt[1] = trim((string)$opt[1]);
					switch ($opt[0])
					{
						case 'audio':
							$audio = $opt[1];
		...
		else if (in_array($type, array('audio/3gpp', 'audio/3gpp2', 'audio/aac', 'audio/x-aac', 'audio/aiff', 'audio/x-aiff', 'audio/mid', 'audio/midi', 'audio/x-midi', 'audio/mpeg', 'audio/x-mpeg', 'audio/mp3', 'x-audio/mp3', 'audio/mp4', 'audio/m4a', 'audio/x-m4a', 'audio/wav', 'audio/x-wav', 'video/3gpp', 'video/3gpp2', 'video/m4v', 'video/x-m4v', 'video/mp4', 'video/mpeg', 'video/x-mpeg', 'video/quicktime', 'video/sd-video')))
		{
			...
			{
				if ($placeholder != "") {
					$embed .= "<embed type=\"$type\" style=\"cursor:hand; cursor:pointer;\" href=\"" . $this->get_link() . "\" src=\"$placeholder\" width=\"$width\" height=\"$height\" autbtay=\"false\" target=\"myself\" controller=\"false\" loop=\"$loop\" scale=\"aspect\" bgcolor=\"$bgcolor\" pluginspage=\"http://www.apple.com/quicktime/download/\"></embed>";
				}
				else {
					$embed .= "<embed type=\"$type\" style=\"cursor:hand; cursor:pointer;\" src=\"" . $this->get_link() . "\" width=\"$width+\" height=\"$height\" autbtay=\"false\" target=\"myself\" controller=\"true\" loop=\"$loop\" scale=\"aspect\" bgcolor=\"$bgcolor\" pluginspage=\"http://www.apple.com/quicktime/download/\"></embed>";
				}
			}
			else
			{
...
class SimplePie_File
{
	...
	function SimplePie_File($url, $timeout = 10, $redirects = 5, $headers = null, $useragent = null, $force_fsockopen = false)
	{
		...
		if (preg_match('/^http(s)?:\/\//i', $url))
		{
			...
			if (extension_loaded('curl') && version_compare(SimplePie_Misc::get_curl_version(), '7.10.5', '>=') && !$force_fsockopen)
			{
				...
				$this->headers = trim((string)curl_exec($fp));
	...
	function parse_headers($headers)
	{
		...
		foreach ($headers as $header)
		{
			...
			$head[strtolower(trim((string)$data[0]))] = trim((string)$data[1]);
		}
		if (preg_match('/HTTP\/[0-9\.]+ ([0-9]+)(.*)$/i', $status, $matches))
		{
			...
			$head['status']['name'] = trim((string)$matches[2]);
		}
		return $head;
	}
}
...
class SimplePie_Locator
{
	...
	function autodiscovery($file)
	{
		...
		foreach ($links as $link)
		{
			...
			{
				$rel = preg_split('/\s+/', strtolower(trim((string)$link['attribs']['REL']['data'])));
				$type = preg_match('/^(application\/rss\+xml|application\/atom\+xml|application\/rdf\+xml|application\/xml\+rss|application\/xml\+atom|application\/xml\+rdf|application\/xml|application\/x\.atom\+xml|text\/xml)(;|$)/msiU', trim((string)$link['attribs']['TYPE']['data']));
				$href = SimplePie_Misc::absolutize_url(trim((string)$link['attribs']['HREF']['data']), $this->file->url);
	...
	function get_links($file)
	{
		...
		foreach ($links as $link)
		{
			...
			{
				$href = trim((string)$link['attribs']['HREF']['data']);
	...
	function start_handler($parser, $name, $attribs)
	{
		...
		switch ($this->tag_name)
		{
			...
			case $this->namespaces['rss2'] . ':RSS':
				...
				{
					$this->data['feedinfo']['version'] = trim((string)$attribs['VERSION']);
				}
				break;
...
class SimplePie_Sanitize
{
					...
					foreach ($item['author'] as $author)
					{
						...
						if (!empty($author['rss']))
						{
							...
							if (preg_match('/(.*)@(.*) \((.*)\)/msiU', $sane, $matches)) {
								...
								$email = trim((string)"$matches[1]@$matches[2]");
							} else {
								$email = $sane;
							}
						}
	...
	function sanitize($data, $attribs, $is_url = false, $force_decode = false)
	{
		...
		if ($this->strip_attributes)
		{
			...
			{
				$data = preg_replace('/ '. trim((string)$attrib) .'=("|&quot;)(\w|\s|=|-|:|;|\/|\.|\?|&|,|#|!|\(|\)|\'|&apos;|<|>|\+|{|})*("|&quot;)/i', '', $data);
				$data = preg_replace('/ '. trim((string)$attrib) .'=(\'|&apos;)(\w|\s|=|-|:|;|\/|\.|\?|&|,|#|!|\(|\)|"|&quot;|<|>|\+|{|})*(\'|&apos;)/i', '', $data);
				$data = preg_replace('/ '. trim((string)$attrib) .'=(\w|\s|=|-|:|;|\/|\.|\?|&|,|#|!|\(|\)|\+|{|})*/i', '', $data);
			}
		}
