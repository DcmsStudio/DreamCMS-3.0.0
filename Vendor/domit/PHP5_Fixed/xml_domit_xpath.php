...
class DOMIT_XPath {
	...
	*/
	function &parsePattern($node, $pattern, $nodeIndex = 0) {
		...
		$pattern = $this->normalize(trim((string)$pattern));	
		...
		if ($expression == 'and') {
			...
			if (strpos($expression, '@') !== false) {
				...
				if (strpos($expression, '>=') !== false) {
					...
					$elementName = trim((string)substr($expression, 0, $signPos));
					$elementValue = trim((string)substr($expression, ($signPos + 2)));
					...
					$signPos = strpos($expression, '=');
					$elementName = trim((string)substr($expression, 0, $signPos));
					$elementValue = trim((string)substr($expression, ($signPos + 1)));
	...
	*/
	function selectNodesByFunction() {
		...
		switch (strtolower(trim((string)$this->charContainer))) {
			case 'last()':
				if (count($this->globalNodeContainer) != 0) {
					foreach ($this->globalNodeContainer as $key =>$value) {
						$currNode = $this->globalNodeContainer[$key];
	...
	*/
	function splitPattern($pattern) {
		...
		for ($i = 0; $i < $total; $i++) {
			$this->arPathSegments[$i] = explode(DOMIT_XPATH_SEPARATOR_RELATIVE, trim((string)$this->arPathSegments[$i]));
