...
class php_http_client_generic extends php_http_request {
			...
			while (!feof($connResource)) {
				...
				switch ($readState) {
					...
					case HTTP_READ_STATE_HEADERS:
						if (trim((string)$data) == '') { //end of headers is signalled by a blank line
	...
	*/
	function decodeChunkedData($data) {
		...
		$chunkLength = hexdec(trim((string)$chunkLengthInHex));
...
class php_http_response {
	...
	*/
	function setUnformattedHeader($headerString) {
		...
		if ($colonIndex !== false) {
			$key = trim((string)substr($headerString, 0, $colonIndex));
			$value = trim((string)substr($headerString, ($colonIndex + 1)));
