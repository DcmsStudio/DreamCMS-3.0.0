...
if (TinyMCE_Compressor::getParam("js")) {
    ...
    if (TinyMCE_Compressor::getParam("languages")) {
        $data['languages'] = TinyMCE_Compressor::getParam("languages");
    }

    if (TinyMCE_Compressor::getParam("plugins")) {
        $data['plugins'] = TinyMCE_Compressor::getParam("plugins");
    }

    if (TinyMCE_Compressor::getParam("themes")) {
        $data['themes'] = TinyMCE_Compressor::getParam("themes");
    }

    if (TinyMCE_Compressor::getParam("expires")) {
        $data['expires'] = TinyMCE_Compressor::getParam("expires");
    }

    if (TinyMCE_Compressor::getParam("disk_cache")) {
        $data['disk_cache'] = TinyMCE_Compressor::getParam("disk_cache");

        Library::makeDirectory( CACHE_PATH .'tinymce_cache/');
        $data['cache_dir'] = CACHE_PATH .'tinymce_cache/';
    }

	// Default settings
	$tinyMCECompressor = new TinyMCE_Compressor($data);
