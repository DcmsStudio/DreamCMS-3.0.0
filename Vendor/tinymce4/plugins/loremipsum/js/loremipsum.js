...
var w = getWin();
var editor = w.tinymce.EditorManager.activeEditor;
...
var LoremIpsumDialog = {
	init: function ()
	{
		...
		for ( var i = 0; i < arrSentences.length; i++ ) {
			...
			option.appendChild(document.createTextNode(arrLoremIpsumLangs[i] + ': ' + tmpText));
			sentences.appendChild(option);
			//sentences.appendChild(option);
			//sentences.options[i] = option; //new Option( arrLoremIpsumLangs[i] + ': ' + tmpText, i );
		}
	...
	insert: function ()
	{
		...
		return insertedText;
		editor.execCommand( 'mceInsertContent', false, insertedText );
