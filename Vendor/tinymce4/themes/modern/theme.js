...
tinymce.ThemeManager.add('modern', function(editor) {
	...
	function renderInlineUI(args) {
		var panel, panelStatus, inlineToolbarContainer;
		...
		function render() {
			...
			if (settings.statusbar !== false || settings.inlinestatusbar === true ) {
				panel.add({type: 'panel', classes: 'statusbar', layout: 'flex', border: '1 0 0 0', items: [
					{type: 'elementpath'}
				]});
			}
			*/
		...
		//	panelStatus.renderAfter(editor.getContainer());
