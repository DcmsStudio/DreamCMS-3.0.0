...
define("tinymce/dom/StyleSheetLoader", [], function() {
    ...
    var cssLoadCache = []; // patch for dreamcms
	...
	return function(document, settings) {
		...
		function load(url, loadedCallback, errorCallback) {
            ...
            // patch for dreamcms
            if (typeof cssLoadCache[url] != 'undefined'){
                if (loadedCallback) {
                    loadedCallback();
                }
                return;
            ...
            cssLoadCache[url] = true; // patch for dreamcms
		...
		insert: function(node, ref_node, before) {
			...
			if (before) {
				...
				if (ref_node === parent.lastChild) {
					...
					if (ref_node.next != null && typeof ref_node.next.prev != 'undefined') {
...
define("tinymce/html/Writer", [
	 ...
	 */
	return function(settings) {
		var html = [], indent, indentBefore, indentAfter, encode, htmlOutput, level = 0;
...
/*
				if ((!empty || htmlOutput) && indent && indentAfter[name] && html.length > 0) {
					value = html[html.length - 1];
				...
				else if (empty && indent && indentAfter[name] && html.length > 0) {
					value = html[html.length - 1];
			 ...
			 */
				...
				var value, skipIndent = false;
				var t = html;
				var testStr = t.join('' ), re = new RegExp('^\s*<'+ name +'([^>]*)>', 'g');


				if (indent && html.length > 0) {

					var v1 = html[html.length - 1];
					var v2 = html[html.length - 2];
					if (v1 == '\n' && v2.length > 0) {
						if (re.test(v2)) {
							skipIndent = true;
						}
				...
				if (indent && html.length > 0 && !skipIndent) {
					for (var x = 0; x<level;x++) {
						html.push('    ');
					}
				}
			...
			text: function(text, raw) {
				...
				if (text.length > 0) {
					...
					var tt = text.replace(/^\s*/g, '').replace(/\s*$/g, '');


					if (indent && html.length > 0 && !raw && tt.length > 0 ) {
						for (var x = 0; x<level;x++) {
							html.push('    ');
						}
					}

					var v = raw ? text : encode(text ).replace(/^\s*/g, '').replace(/\s*$/g, '');
					html.push(v);
					if (indent && v.length > 0 && !v.match(/\n$/)) {
						html.push('\n');
					}
				}
			},
			 ...
			 */
			cdata: function(text) {
				if (indent && html.length > 0 && text.length > 0) {
					for (var x = 0; x<level;x++) {
						html.push('    ');
					}
				}
...
define("tinymce/ui/Control", [
	...
	var Control = Class.extend({
		...
		renderAfter: function(elm) {
			var self = this;

			elm = elm || self.getContainerElm();
			elm.parentNode.insertBefore(DomUtils.createFragment(self.renderHtml()), elm.nextSibling);
			self.postRender();
		...
		init: function(settings) {
			...
			self.on('postrender show', function(e) {
				if (e.control == self) {
					...
					if (self.modal && !hasModal && !settings.modal) {
...
define("tinymce/util/I18n", [], function() {
	...
	return {
		...
		add: function(code, items) {
			...
			var st = '';
			if (typeof(code) == "string" && code.match(/([\.])/g) ) {
