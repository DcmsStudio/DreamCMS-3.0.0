...
window.CodeMirror = (function() {
    ...
    function CodeMirror(place, options) {
        ...
        var display = this.display = makeDisplay(place, this.options);
    ...
    function makeDisplay(place, options) {
        var d = {};
        ...
        d.errorCounter = 0;
        if (options.gutterPosRight)
        {
            d.scroller.style.right = d.gutters.style.width;
            d.scrollbarV.style.right = d.gutters.style.width;
        }
        return d;
    }
    ...
    function wrappingChanged(cm) {
        ...
        if (cm.options.lineWrapping) {
            ...
            doc.iter(0, doc.size, function(line) {
                ...
                if (guess != 1)
                    updateLineHeight(line, guess * th);
            });
            ...
            doc.iter(0, doc.size, function(line) {
                if (line.height != 0)
                    updateLineHeight(line, th);
            });
        }
        regChange(cm, 0, doc.size);
        clearCaches(cm);
        setTimeout(function() {
            updateScrollbars(cm.display, cm.view.doc.height, cm);
        }, 100);
    }
    ...
    function updateScrollbars(d /* display */, docHeight, cm) {
        var totalHeight = docHeight + (2 * paddingTop(d));
        d.sizer.style.minHeight = d.heightForcer.style.top = totalHeight + "px";


        //if( !d.sizer.style.minHeight ) d.sizer.style.minHeight = d.heightForcer.style.top = totalHeight + "px";
        var scrollHeight = Math.max(totalHeight, d.scroller.scrollHeight);
        var needsH = d.scroller.scrollWidth > d.scroller.clientWidth;
        var needsV = scrollHeight > d.scroller.clientHeight;
        if (needsV) {
            if (cm.options.gutterPosRight)
            {
                d.scrollbarV.style.right = d.gutters.style.width;
            }
            d.scrollbarV.style.display = "block";
            d.scrollbarV.style.bottom = needsH ? scrollbarWidth(d.measure) + (cm.options.gutterPosRight ? 20 : 0) + "px" : "0";
    ...
    function alignHorizontally(cm) {
        ...
        if (!cm.options.gutterPosRight)
            ...
            display.gutters.style.right = (comp + gutterW) + "px";
        
        if (cm.options.gutterPosRight)
        {
            display.scroller.style.right = gutterW;
            display.scrollbarV.style.right = gutterW;
        }
    ...
    function updateDisplay(cm, changes, viewPort) {
        errorCount = 0;
        cm.display.lineSpace.className = '';
        ...
        /*
         if ( cm.hasError )
         {
         cm.display.lineSpace.className = 'CodeMirror-linenumbers-errors';
         }
         else
         {
         cm.display.lineSpace.className = '';
    ...
    function updateDisplayInner(cm, changes, viewPort) {
        ...
        // Patch by dw2k (DreamCMS 3.0 Merge View)
        if (!cm.options.gutterPosRight)
            display.sizer.style.marginLeft = display.scrollbarH.style.left = display.gutters.offsetWidth + "px";
        else
            display.sizer.style.marginRight = display.scrollbarH.style.left = display.gutters.offsetWidth + "px";
        ...
        if (sawCollapsedSpans) {
            ...
            while (to < doc.size && lineIsHidden(getLine(doc, to)))
                ++to;
        }
    ...
    function patchDisplay(cm, from, to, intact, updateNumbersFrom) {
        ...
        cm.view.doc.iter(from, to, function(line) {
            ...
            if (lineIsHidden(line)) {
                ...
                var lineNode = buildLineElement(cm, line, lineNo, dims, cm.display.errorCounter);
    ...
    function buildLineElement(cm, line, lineNo, dims, container) {
        ...
        if (lineElement.innerHTML.match(/cm-error/))
        {
            error = cm.hasError = hasError = true;
            errorCount++;
            // cm.display.errorCounter++;
            //container.className = ' CodeMirror-linenumbers-errors';
        }
        else
        {
            if (!cm.hasError && errorCount > 0)
                errorCount--;
        }
        ...
        if (cm.options.lineNumbers || markers) {
            ...
            {
                widths += display.lineNumInnerWidth;
            }
            ...
            if (markers)
            {
                for (var k = 0; k < cm.options.gutters.length; ++k) {
                    ...
                    if (found && dims.gutterWidth[id] > 0)
                    {
                        widths += dims.gutterWidth[id];
                    }
                }
            }


            var gutterWrap = wrap.appendChild(elt("div", null, (error ? null : null), "position: absolute; left: " +
                    (dims.fixedPos) + "px;width:" + widths + 'px'));
            ...
            if (cm.options.lineNumbers && (!markers || !markers["CodeMirror-linenumbers"]))
            {
                wrap.lineNumber = gutterWrap.appendChild(
                        ...
                        "CodeMirror-linenumber CodeMirror-gutter-elt" + (error ? ' error' : ''),
            ...
            if (markers)
            {
                for (var k = 0; k < cm.options.gutters.length; ++k) {
                    var id = cm.options.gutters[k], found = markers.hasOwnProperty(id) && markers[id];
                    if (found)
                        gutterWrap.appendChild(elt("div", [found], "CodeMirror-gutter-elt " + id + (error ? ' error' : ''), "left: " +
    ...
    function highlightWorker(cm) {
        ...
        if (view.frontier >= cm.display.showingTo)
            return;
    ...
    function getStateBefore(cm, n) {
        var view = cm.view;
        if (!view.mode.startState)
            return true;
        var pos = findStartLine(cm, n), state = pos && getLine(view.doc, pos - 1).stateAfter;
        if (!state)
            state = startState(view.mode);
        else
            state = copyState(view.mode, state);
    ...
    function charCoords(cm, pos, context, lineObj) {
        if (!lineObj)
            lineObj = getLine(cm.view.doc, pos.line);
    ...
    function onMouseDown(e) {
        ...
        e_preventDefault(e);
        if (type == "single")
            extendSelection(cm, clipPos(doc, start));
        ...
        function doSelect(cur) {
            ...
            if (type == "double") {
                ...
                if (posLess(cur, startstart))
                    ...
                    extendSelection(cm, startstart, word.to);
            } else if (type == "triple") {
                if (posLess(cur, startstart))
                    extendSelection(cm, startend, clipPos(doc, {
                        line: cur.line,
                        ch: 0
                    }));
                else
                    extendSelection(cm, startstart, clipPos(doc, {
                        line: cur.line + 1,
                        ch: 0
                    }));
            }
    ...
    function onDrop(e) {
        ...
        if (files && files.length && window.FileReader && window.File) {
            ...
            if (cm.view.draggingText && !(posLess(pos, cm.view.sel.from) || posLess(cm.view.sel.to, pos))) {
                ...
                if (ie)
                    setTimeout(bind(focusInput, cm), 50);
                return;
            }
            try {
    ...
    function setScrollTop(cm, val) {
        if (Math.abs(cm.view.scrollTop - val) < 2)
            return;
    ...
    function setScrollLeft(cm, val, isScroller) {
        if (isScroller ? val == cm.view.scrollLeft : Math.abs(cm.view.scrollLeft - val) < 2)
            return;
    ...
    function onScrollWheel(cm, e) {
        ...
        if (dy && wheelPixelsPerUnit != null) {
            ...
            if (pixels < 0)
                ...
                bot = Math.min(cm.view.doc.height, bot + pixels + 50);
            updateDisplay(cm, [], {
                top: top,
                bottom: bot
            });
    ...
    function updateDocInner(cm, from, to, newText, selUpdate, origin) {
        ...
        if (view.history)
            addChange(cm, from.line, newText.length, old, origin,
    ...
    function updateDocNoUndo(cm, from, to, lines, selUpdate, origin) {
        ...
        if (from.ch == 0 && to.ch == 0 && hlText(lastHL) == "") {
            ...
            if (nlines)
                doc.remove(from.line, nlines, cm);
            if (added.length)
            ...
            for (var i = 1, e = lines.length - 1; i < e; ++i)
                added.push(makeLine(hlText(lines[i]), hlSpans(lines[i]), th));
            if (nlines > 1)
                doc.remove(from.line + 1, nlines - 1, cm);
            doc.insert(from.line + 1, added);
        }
    ...
    function scrollPosIntoView(cm, pos) {
        for (; ; ) {
            ...
            if (scrollPos.scrollTop != null) {
                ...
                if (Math.abs(cm.view.scrollTop - startTop) > 1)
                    changed = true;
            }
    ...
    CodeMirror.prototype = {
        ...
        getTokenTypeAt: function(pos) {
            ...
            var styles = getLineStyles(this, getLine(this.view.doc, pos.line));
        ...
        addLineWidget: operation(null, function(handle, node, options) {
            ...
            widget.node = node;
            if (widget.noHScroll)
                this.display.alignWidgets = true;
        ...
        lineAtHeight: function(height, mode) {
            ...
            return lineAtHeight(this.view.doc, height + this.display.viewOffset);
        },
        heightAtLine: function(line, mode) {
            var end = false, last = 0 + this.view.doc.size - 1;
            if (line < 0)
                line = 0;
            else if (line > last) {
                line = last;
            ...
            var lineObj = getLine(this.view.doc, line);
            return intoCoordSystem(this, getLine(this.view.doc, line), {top: 0, left: 0}, mode || "page").top +
        ...
        getRange: function(from, to, lineSep) {
            ...
            if (l1 == l2)
                return getLine(doc, l1).text.slice(from.ch, to.ch);
        ...
        deleteH: operation(null, function(dir, unit) {
            ...
            if (!posEq(sel.from, sel.to))
                ...
                replaceRange(this, "", sel.from, findPosH(this, dir, unit, false), "delete");
        ...
        scrollTo: function(x, y) {
            if (x != null)
                this.display.scrollbarH.scrollLeft = this.display.scroller.scrollLeft = x;
            if (y != null)
    ...
    function removeReadOnlyRanges(doc, from, to) {
        ...
        for (var i = 0; i < markers.length; ++i) {
            ...
            for (var j = 0; j < parts.length; ++j) {
                var p = parts[j];
                if (!posLess(m.from, p.to) || posLess(m.to, p.from))
                    continue;
                var newParts = [j, 1];
    ...
    function updateLine(cm, line, text, markedSpans) {
        ...
        if (lineIsHidden(line))
            ...
            line.height = textHeight(cm.display);
