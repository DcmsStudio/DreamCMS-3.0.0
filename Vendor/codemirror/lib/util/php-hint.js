(function() {
    ...
    function scriptHint(editor, keywords, getToken) {
        ...
        var lineCode = editor.getTokenAt({line: cur.line, ch: cur.ch});
        ...
        return {
            list: getCompletions(token, context, keywords, editor.getValue(), lineCode.string),
            from: {
                line: cur.line,
                ch: token.start
            },
    ...
    var phpSystemVars = ('_POST _GET _REQUEST _GLOBALS _SERVER _COOKIE HTTP_SERVER_VARS HTTP_GET_VAR HTTP_POST_VARS this').split(" ");

    function getCompletions(token, context, keywords, code, linecode) {
        ...
        var firstChr = start.substr(0, 1);
        //console.log('first:' + firstChr);
        if (firstChr == '$')
        {
            //  length = length - 1;
        }
        ...
        function maybeAdd(str) {
            if (firstChr === '$' && '('.indexOf(str) > -1 || str == start) {
                return;
            }
        ...
        function isPhpSystemVars(str)
        {
            forEach(phpSystemVars, function(item) {
                if (item == str)
                {
                    return true;
                }
        ...
        if (code)
        {
            ...
            {
                // match code vars :)
                var strVars = '$' + phpSystemVars.join(',$');
                var tmpSys = strVars.split(',');
                
                
                while (match = re.exec(code)) {
                    if (!tmp[match[1]]) {
                        //maybeAdd( match[1] );
                        if (!arrayContains(phpSystemVars, match[1]))
                            params.push(match[1]);
                        tmp[match[1]] = true;
                    }
                }
            ...
            if (firstChr != '$')
            {
                ...
                // match code static classes :)
                re = /new\s\s*([a-zA-Z0-9_]+)(\s*\(|;)/g;
                while (match = re.exec(code)) {
                    if (!tmp[match[1]]) {
                        //maybeAdd( match[1] );
                        params.push(match[1] + '()');
            ...
            forEach(params, maybeAdd);

            params = tmp = null;
        }


        if ('::'.indexOf(linecode) > -1 || '->'.indexOf(linecode) > -1)
        {
            return found;
        }
