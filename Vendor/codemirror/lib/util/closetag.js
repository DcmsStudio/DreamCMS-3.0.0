...
(function() {
     ...
     */
    CodeMirror.defineExtension("closeTag1", function(cm, ch, indent) {
        ...
        if (!configSet && typeof top.dcms_tags == 'object' && (mode == 'text/html' || mode == 'application/xml') ) {
            for (tagName in top.dcms_tags) {
                if (top.dcms_tags[tagName].isSingleTag || top.dcms_tags[tagName].singleTag )
                {
                    if (CodeMirror.defaults['closeTagVoid'] && tagName != 'cp:block' ) {
                        CodeMirror.defaults['closeTagVoid'].push(tagName);
                    }
                }
                else if (!top.dcms_tags[tagName].isSingleTag && !top.dcms_tags[tagName].singleTag)
                {
                    if (CodeMirror.defaults['closeTagIndent']  ) {
                        CodeMirror.defaults['closeTagIndent'].push(tagName);
                    }
                }
            }
        ...
        if (mode == 'text/html' || mode == 'application/xml') {
            ...
            if (ch == '>') {
                ...
                if ((tok.className == 'tag' || tok.className == 'cptag') && type == 'closeTag') {
                ...
                if ((tok.className == 'tag' || tok.className == 'cptag') && type != 'selfcloseTag') {
                ...
                if ((tok.className == 'tag' || tok.className == 'cptag') && tok.string == '<') {
...
(function() {
    ...
    function autoCloseTag(cm, ch) {
        ...
        var opt = cm.getOption("autoCloseTags"), html = inner.mode.configuration == "html", xml = inner.mode.configuration == "xml";

        if (typeof top.dcms_tags == 'object') {
            for (tagName in top.dcms_tags) {
                if (top.dcms_tags[tagName].isSingleTag || top.dcms_tags[tagName].singleTag)
                {
                    if (typeof opt == "object" && opt.dontCloseTags) {
                        opt.dontCloseTags.push(tagName);
                    }
                    else if ((html || xml) && htmlDontClose) {
                        htmlDontClose.push(tagName);
                    }
                }
            }
        }

        var dontCloseTags = (typeof opt == "object" && opt.dontCloseTags) || ((html || xml) && htmlDontClose);
        var indentTags = (typeof opt == "object" && opt.indentTags) || ((html || xml) && htmlIndent);
        ...
        if (ch == ">" && state.tagName) {
            ...
            return;
        } else if (ch == "/" && (tok.type == "tag" || tok.type == "cptag") && tok.string == "<") {
