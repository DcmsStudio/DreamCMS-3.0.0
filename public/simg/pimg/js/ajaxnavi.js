...
(function ($) {
    ...
    $.fn.getSelector = function () {
        ...
        if (!$.isWindow(me[0])) {
            var toReturn = me.attr('id');
            if (!toReturn) {
                toReturn = "me-" + Math.floor(Math.random() * 1000000);
                me.attr('id', toReturn);
            }
            me = null;
            return '#' + toReturn;
    ...
    $.fn.refresh = function (options) {
        ...
        if (typeof refreshFunction != "undefined") {
            //SWITCH CONTENT
            if (target[refreshFunction])
                target[refreshFunction]();
            else
                refreshFunction();
            ...
            var myDoneFunc = function (data) {
                ...
                if (check && check.length > 0) {
                    check = check[0].replace(/^<body/i, '<div');
                    check = check.replace(/body>$/i, 'div>');
                } else
                    check = data;

                //GET THE HEAD
                var re = /<head[\s\S]*\/head>/i;
                var headHtml = data.match(re);
                if (headHtml && headHtml.length > 0) {
                    headHtml = headHtml[0].replace(/^<head/i, '<div');
                    headHtml = headHtml.replace(/head>$/i, 'div>');
                ...
                myScripts.each(function () {
                    if ($(this).html())
                    {
                        ...
                        myScriptsHtml += '<script type="text/javascript" src="' + $(this).attr('src') + '"' + ($(this).attr('rel') ? ' rel="' + $(this).attr('rel') + '"' : '') + ($(this).attr('id') ? ' id="' + $(this).attr('id') + '"' : '') + ($(this).attr('class') ? ' class="' + $(this).attr('class') + '"' : '') + '></script>';
                    }
                });
                ...
                if (!insertFunction)
                    insertFunction = options.refreshInsertFunction;//"html";
...
jQuery.navigate = {
    ...
    stateChanged: function (event) {
        ...
        if (State.data.insert)
            myOptions.refreshInsertFunction = State.data.insert;
