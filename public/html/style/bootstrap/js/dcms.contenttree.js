...
ContentTree = {
    ...
    getIndent: function (level, foritem)
    {
        if (foritem) {
            return $('<div/>').css({
                width: (parseInt(15 * level, 0) || 0)
            }).addClass('tree-indenter2').append('<span><span></span></span>');
        }
        return $('<div/>').css({
            width: (parseInt(15 * level, 0) || 0)
    ...
    getNodeIcon: function (icon, nodedata)
    {
        ...
        if (typeof nodedata == 'object')
        {
            ...
            {
				sprite = 'clock document-clock';
                icon = Config.get('backendImagePath', '') + 'tree/document-clock.png';
                if (locked)
                {
					sprite = 'locked document-locked';
            ...
            if (typeof nodedata.isindexpage != 'undefined' && nodedata.isindexpage == 1)
            {
                ...
                if (locked)
                {
					sprite = 'locked page-index-locked';
            ...
            if (typeof nodedata.is_folder != 'undefined' && nodedata.is_folder == 1)
            {
                ...
                if (locked)
                {
					sprite = 'locked folder-closed-locked';
		...
		return $('<i class="tree-node-icon ts '+sprite+'"></i>');
	...
	toggleNodeIcon: function (state, nodeIcon)
	{
		if (nodeIcon.hasClass('tree-node-icon')) {

			var cls = nodeIcon.attr('class' ).trim();

			if (state == 'locked') {
				if (cls.match(/\sdocument\-clock$/)) {
					nodeIcon.removeClass('document-clock').addClass('locked document-locked');
				}
				else if (cls.match(/\spage\-index$/)) {
					nodeIcon.removeClass('page-index').addClass('locked page-index-locked');
				}
				else if (cls.match(/\spage$/)) {
					nodeIcon.removeClass('page').addClass('locked page-locked');
				}
				else if (cls.match(/\sfolder\-closed$/)) {
					nodeIcon.removeClass('folder-closed').addClass('locked folder-closed-locked');
				}
				else if (cls.match(/\sfolder\-open$/)) {
					nodeIcon.removeClass('folder-open').addClass('locked folder-open-locked');
				}
			}
			else if (state == 'unlocked') {
				if (cls.match(/\sclock\sdocument\-locked$/)) {
					nodeIcon.removeClass('document-locked locked').addClass('document-clock');
				}
				else if (cls.match(/\spage\-index\-locked$/)) {
					nodeIcon.removeClass('page-index-locked locked').addClass('page-index');
				}
				else if (cls.match(/\spage\-locked$/)) {
					nodeIcon.removeClass('page-locked locked').addClass('page');
				}
				else if (cls.match(/\sfolder\-closed\-locked$/)) {
					nodeIcon.removeClass('folder-closed-locked locked').addClass('folder-closed');
				}
				else if (cls.match(/\sfolder\-open\-locked$/)) {
					nodeIcon.removeClass('folder-open-locked locked').addClass('folder-open');
				}
			}
    ...
    getEmptyNode: function (container, level)
    {
        ...
        div.append(this.getIndent(level, true));


		div.append(this.getNodeIcon(null, {published: 1}).css({
			opacity: .5
		}));
		/*
    ...
    updateIndenter: function (node, container)
    {
        ...
        if (!tonode.hasClass('isFolder') || !node.hasClass('tree-node'))
        {
            return;
        }

        var level = parseInt(tonode.attr('level')) || 1;
        ...
        {
            var newIndent = this.getIndent(itemNewLevel + 1, true);
    ...
    {
        var self = this, appendTo = ( $('#fullscreenContainer' ).length == 1 ? $('#fullscreenContainer' ) : 'body' );
                ...
                if (allowMove && typeof allowMove.moveitem === 'string' || $(this).attr('level') == 0)
                {
                    $(this).addClass('can-drop').droppable({
                        ...
                        drop: function (event, ui) {
                            ...
                            if (event.originalEvent.type == 'mouseup' && $(this ).hasClass('tree-node') )
                            {
                                var $to = $(this).next();
	...
	bindNodeEvents: function() {
		...
		var self = this, nodes = $(this.el ).find('div.tree-node' );

		nodes.unbind('click.pagestreeSelectNode');
		nodes.each(function() {

			$(this ).bind('click.pagestreeSelectNode', function (e) {
				self.selectNode(this);
			});

			if ($(this ).hasClass('isFolder')) {

				$(this ).find('a[rel=treetoggle]' ).unbind().bind('click', function (e)
				{
					e.preventDefault();

					if ($(e.target).parents('.tree-node:first' ).length == 1) {
						var node = $(e.target).parents('.tree-node:first' ).data('nodeData');
    ...
    rebuildTree: function (froceReload)
    {
        ...
        this.getData(force, function (data) {
            if (Tools.responseIsOk(data))
            {
				_self.createTree(data);
            }
            else
            {
                $(_self.el).unmask();
                _self.bindDraggable();
				_self.bindNodeEvents();
	...
	createTree: function(data)
	{
		...
		if (last_node.id)
		{
			...
			ancestors.each(function () {
				...
				if (pm_icon.length == 1) {
					...
					pm_icon.next().attr('class', pm_icon.next().attr('class' ).replace('folder-closed', 'folder-open') );
		...
		if ( first_node ) $('#tree-node-' + first_node.id).addClass('tree-node-active');
		$(el).unmask();
		_self.contextMenu();
		_self.bindDraggable();
		_self.bindNodeEvents();
    ...
    buildTree: function (data, openTo) {
        ...
        if (last_node.id) {
            var ancestors = $('#tree-node-' + last_node.id).parents('.nodes-container');
            ancestors.each(function () {
                var pm_icon = $(this).prev().children('a').find('img:first');
                if (pm_icon.length == 1) {
                    pm_icon.attr({
        ...
        setTimeout(function () {
            $('#panel-sites .panel-content').nanoScroller({
                scrollContent: $('#panel-sites .panel-content')
            });
        }, 150);
    },
    buildNode: function (node, idx)
    {
        ...
        if (typeof node.is_folder != 'undefined' && node.is_folder == 1)
        {
            ...
            var toggler = $('<a>').attr({
                href: '#'
            } ).attr('rel', 'treetoggle').attr('id', 'm' + node.module + '_a' + parseInt(node.appid) + '_' + node.id);


            toggler.bind('click.pagestreeToggle', function (e)
            {
                e.preventDefault();
            ...
            div.unbind().css('cursor', 'pointer').bind('click.pagestreeSelectNode', function (e) {
           //     if ($(e.target).parents('.tree-node:first') || $(e.target ).hasClass('tree-node') ) {
                    self.selectNode(this);
            //    }
        ...
        {
            div.append(this.getIndent((level ), 'page'));
            div.append(this.getNodeIcon(null, node));

			div.unbind('click.pagestreeSelectNode').bind('click.pagestreeSelectNode', function (e) {
                e.preventDefault();
				//if ($(e.target).parents('.tree-node:first') || $(e.target ).hasClass('tree-node') ) {
					self.selectNode($(this));
				//}
			} ).unbind('dblclick.pagestreeNode').bind('dblclick.pagestreeSelectNode', function (e) {
                    e.preventDefault();
					self.selectNode($(this));

					if ( $(this ).data('nodeData') ) {
						var data = $(this ).data('nodeData');
						if ( typeof data.actions['edit-item'] != 'undefined') {
							var url = data.actions['edit-item'].replace('{contentid}', data.contentid ).replace('{catid}', data.catid );


							openTab({url: 'admin.php?'+url, obj: $(this ), isTreeNode: true});
    ...
    reloadNode: function ()
    {
		...
		*/
        ...
        $.ajax({
            url: 'admin.php?adm=dashboard&action=contenttree&modul=' + modul + '&modulid=' + modulid + '&catid=' + catid + '&level=' + level,
            method: 'GET',
            async: true,
            cache: false,
    ...
    nodeToggle: function (el)
    {
        ...
        if (container.is(':visible'))
        {
			...
			if ( nodeIcon.hasClass('lock'))
			{
				nodeIcon.removeClass('folder-open-locked').addClass('folder-closed-locked');
			}
			else {
                        ...
                        if (typeof _data.modules != 'undefined')
                        {
                            ...
                            setTimeout(function () {
                                ...
                                setTimeout(function () {
									...
									if ( nodeIcon.hasClass('lock'))
									{
										nodeIcon.removeClass('folder-closed-locked').addClass('folder-open-locked');
									}
									else {
...
<li><a href="#lock">Document Lock</a></li>';
        ...
        el.contextMenu({
            ...
            onBeforeShow: function (e, menuObj, callbackContext)
            {
                ...
                if (selNode.hasClass('locked') || selNode.find('i.tree-node-icon').hasClass('locked') )
                {
        ...
        function (action, _el, pos, e) {
            ...
            if (action === 'insert-dynamic-link-content')
            {
                ...
                if (typeof napptype != 'undefined' && napptype != '')
                {
                    ...
                    str = 'data-isapp="true" data-modul="' + napptype + '" data-contentid="' + idofContent + '"';
                    Doc.doInsertRichText(true, str, nodeLabel, tag);
                }
                else
                {
                    tag = '{' + nmodul + ':' + idofContent + '}';
                    str = 'data-modul="' + nmodul + '" data-contentid="' + idofContent + '"';
                    Doc.doInsertRichText(true, str, nodeLabel, tag);
                }
            }
            else if (action === 'insert-static-link-content')
            {
                ...
                {
                    tag = '{' + napptype + ':' + idofContent + '}';
                    str = 'data-isapp="true" data-modul="' + napptype + '" data-contentid="' + idofContent + '" data-static="true"';
                    Doc.doInsertRichText(true, str, nodeLabel, tag);
                }
                else
                {
                    tag = '{' + nmodul + ':' + idofContent + '}';
                    str = 'data-modul="' + nmodul + '" data-contentid="' + idofContent + '" data-static="true"';
            ...
            else if (action === 'unlock' || action === 'lock')
            {
                ...
                if (idofContent && nmodul)
                {
                    ...
                    var nodeIcon = $('i.tree-node-icon:first', selNode);
...
*/
                        ...
                        Doc.unlock(idofContent, nmodul, lockaction, pk, table, function () {
							...
							self.toggleNodeIcon('unlocked', nodeIcon);
...
*/
                        ...
                        Doc.lock(idofContent, nmodul, lockaction, pk, table, title, editlocation, function () {
                            //nodeIcon.attr('src', nodeIcon.attr('oldIcon').replace('.', '-locked.'));
							//nodeIcon.attr('class', nodeIcon.attr('class') );
							self.toggleNodeIcon('locked', nodeIcon);
