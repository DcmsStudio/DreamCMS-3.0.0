







(function($) {

	var adReplace = ''; //'<strong>Ad removed by dcmsAdBlock</strong>';
	var adFilters = [

						"&ad_keyword=",
						"&ad_type_",
						"&adname=",
						"&adspace=",
						"&adtype=",
						"&advertiserid=",
						"&clientype=*&adid=",
						"&googleadword=",
						"adverts/",
						"-ad-large.",
						"-ad-loading.",
						"-ad-manager/",
						"-ad-util-",
						"-ad.html",
						"-banner-ads/",
						"-content/adsys/",
						"-rebels-ads/",
						"-text-ads.",
						"-third-ad.",
						".ad.footer.",
						".ad.premiere.",
						".adplacement=",
						".ads.darla.",
						".adserver.",
						".aspx?zoneid=*&task=",
						".au/_ads/",
						".au/ads/",
						".ca/ads/",
						".com/ad-",
						".com/ad.",
						".com/ad/",
						".com/ad1.",
						".com/ad2.",
						".com/ad2/",
						".com/ad?",
						".com/ad_",
						".com/ads.",
						".com/ads/",
						".com/ads?",
						".com/adx/",
						".com/gad/",
						".com/gads/",
						".com/openx/*",
						".com/ss/ad/",
						".html?ad=",
						".jp/ads/",
						".net/_ads/",
						".net/ad/$~object_subrequest",
						".net/ads/",
						".net/ads2/",
						".net/ads3/",
						".nz/ads/",
						".org/ad2_",
						".org/ad_",
						".org/ads/",
						".org/gads/",
						".php?bannerid=",
						".php?zoneid=*&loc=",
						".swf?clicktag=",
						".tv/ads/",
						".uk/ads/",
						".us/ads/",
						".za/ads/",
						"/*;cue=pre;$object_subrequest",
						"/.adserv/*",
						"/468xads.",
						"/_media/ads/*",
						"/a8-ads/*",
						"/aamsz=*/acc_random=",
						"/aamsz=*/pageid=",
						"/aamsz=*/position=",
						"/abm.asp?z=",
						"/abmw.asp?z=",
						"/abmw.aspx",
						"/acc_random=*/aamsz=",
						"/ad-1.5.",
						"/ad-amz.",
						"/ad-banner-",
						"/ad-box-",
						"/ad-cdn.",
						"/ad-frame.",
						"/ad-hug.",
						"/ad-iframe-",
						"/ad-inject/*",
						"/ad-leaderboard.",
						"/ad-loader-",
						"/ad-managment/*",
						"/ad-server/*",
						"/ad-top-",
						"/ad-topbanner-",
						"/ad-vert.",
						"/ad-vertical-",
						"/ad.asp?",
						"/ad.aspx",
						"/ad.cgi?",
						"/ad.css?",
						"/ad.html",
						"/ad.jsp?",
						"/ad.mason?",
						"/ad.php?",
						"/ad.pl?z",
						"/ad/?id=",
						"/ad/adtech",
						"/ad/banner",
						"/ad/code",
						"/ad/common",
						"/ad/frame",
						"/ad/google",
						"/ad/header",
						"/ad/iframe/*",
						"/ad/im.js",
						"/ad/img/*",
						"/ad/inad",
						"/ad/init",
						"/ad/lrec",
						"/ad/mercury",
						"/ad/mrec",
						"/ad/script/*",
						"/ad/serve",
						"/ad/side_",
						"/ad/skyscraper",
						"/ad/sponsor",
						"/ad/takeover/*",
						"/ad/text",
						"/ad/view",
						"/ad/wr.php",
						"/ad160.php",
						"/ad1x1home.",
						"/ad2.aspx",
						"/ad2.html",
						"/ad300x145.",
						"/ad350.html",
						"/ad728.php",
						"/ad728x15.",
						"/ad?count=",
						"/ad_agency/*",
						"/ad_banner/*",
						"/ad_banner_",
						"/ad_configuration.",
						"/ad_creatives.",
						"/ad_editorials_",
						"/ad_engine?",
						"/ad_feed.js?",
						"/ad_forum_",
						"/ad_frame.",
						"/ad_function.",
						"/ad_functions.",
						"/ad_google.",
						"/ad_holder/*",
						"/ad_iframe.",
						"/ad_iframe_",
						"/ad_insert.",
						"/ad_jnaught/*",
						"/ad_js.php",
						"/ad_label_",
						"/ad_leader.",
						"/ad_left.",
						"/ad_legend_",
						"/ad_link.",
						"/ad_manager.",
						"/ad_notice.",
						"/ad_os.php?",
						"/ad_page_",
						"/ad_rectangle_",
						"/ad_refresher.",
						"/ad_reloader_",
						"/ad_right.",
						"/ad_rotation.",
						"/ad_rotator_",
						"/ad_serv.",
						"/ad_serve.",
						"/ad_server.",
						"/ad_server/*",
						"/ad_sizes=",
						"/ad_skin_",
						"/ad_sky.",
						"/ad_slideout.",
						"/ad_space.",
						"/ad_square.",
						"/ad_supertile/*",
						"/ad_tag_",
						"/ad_tile/*",
						"/ad_title_",
						"/ad_top.",
						"/ad_topgray2.",
						"/ad_tpl.",
						"/ad_vert.",
						"/ad_view_",
						"/adaffiliate_",
						"/adbanner.",
						"/adbanner/*",
						"/adbanners/*",
						"/adbar.aspx",
						"/adbot_promos/*",
						"/adbox.gif",
						"/adbrite.",
						"/adbureau.",
						"/adcell/*",
						"/adcentral.",
						"/adchannel_",
						"/adclick.",
						"/adclient.",
						"/adclient/*",
						"/adclutter.",
						"/adcode.",
						"/adcodes/*",
						"/adconfig.js",
						"/adconfig.xml?",
						"/adconfig/*",
						"/adcontent.$~object_subrequest",
						"/adcontroller.",
						"/adcreative.",
						"/adcycle.",
						"/adcycle/*",
						"/addelivery/*",
						"/addyn/3.0/*",
						"/addyn*adtech;",
						"/adengage1.",
						"/adengage_",
						"/adengine/*",
						"/adexclude/*",
						"/adf.cgi?",
						"/adfarm.",
						"/adfetch?",
						"/adfetcher?",
						"/adfile/*",
						"/adfooter.",
						"/adframe.",
						"/adframe/*",
						"/adframe?",
						"/adframe_",
						"/adfshow?",
						"/adfunction.",
						"/adfunctions.",
						"/adgraphics/*",
						"/adguru.",
						"/adhandler.",
						"/adhandler/*",
						"/adheader.",
						"/adhomepage.",
						"/adhtml/*",
						"/adhug_jc.",
						"/adiframe.",
						"/adiframe/*",
						"/adimage.",
						"/adimages.",
						"/adimages/*",
						"/adindex/*",
						"/adinjector.",
						"/adinsert.",
						"/adinterax.",
						"/adjs.php?",
						"/adjsmp.",
						"/adlabel.",
						"/adlabel_",
						"/adlayer.",
						"/adlayer/*",
						"/adleader.",
						"/adlink.",
						"/adlink_",
						"/adlinks.",
						"/aad/*",
						"/adman.js",
						"/adman/image/*",
						"/adman/www/*",
						"/admanagement/*",
						"/admanagementadvanced.",
						"/admanager.$~object_subrequest",
						"/admanager/*$~object_subrequest",
						"/admanager3.",
						"/admanagers/*",
						"/admanagerstatus/*",
						"/admaster.",
						"/admaster?",
						"/admedia.",
						"/admega.",
						"/admentor/*",
						"/admin/ad_",
						"/adnetmedia.",
						"/adnetwork.",
						"/adnews.",
						"/adng.html",
						"/adonline.",
						"/adpage.",
						"/adpage/*",
						"/adparams.",
						"/adpeeps.",
						"/adpeeps/*",
						"/adplayer.",
						"/adplayer/*",
						"/adplugin.",
						"/adpoint.",
						"/adpopup.",
						"/adproducts/*",
						"/adproxy.",
						"/adproxy/*",
						"/adrelated.",
						"/adreload?",
						"/adremote.",
						"/adrevenue/*",
						"/adrevolver/*",
						"/adriver_",
						"/adrolays.",
						"/adroot/*",
						"/adrot.js",
						"/adruptive.",
						"/ads-1.js",
						"/ads-2.js",
						"/ads-banner.",
						"/ads-common.",
						"/ads-leader",
						"/ads-rec",
						"/ads-service.",
						"/ads-sky",
						"/ads.asp?",
						"/ads.dll/*",
						"/ads.htm",
						"/ads.php?",
						"/ads/acctid",
						"/ads/ad-",
						"/ads/cnvideo/*",
						"/ads/common/*",
						"/ads/footer_",
						"/ads/frame",
						"/ads/freewheel/*",
						"/ads/home/*",
						"/ads/house/*",
						"/ads/html",
						"/ads/i_ads.",
						"/ads/interstitial/*",
						"/ads/java",
						"/ads/labels/*",
						"/ads/layer.",
						"/ads/pa/*",
						"/ads/paid",
						"/ads/right",
						"/ads/text",
						"/ads/third-party/*",
						"/ads/top",
						"/ads/www",
						"/ads2.php",
						"/ads2_2.",
						"/ads_global.",
						"/ads_iframe.",
						"/ads_php/*",
						"/ads_reporting/*",
						"/ads_v2.",
						"/ads_yahoo.",
						"/adsa468.",
						"/adsa728.",
						"/adsadclient31.",
						"/adsadview.",
						"/adsatt.",
						"/adsbanner.",
						"/adscluster.",
						"/adscontent.",
						"/adscontent2.",
						"/adscript.",
						"/adscripts/*",
						"/adscroll.",
						"/adsearch.",
						"/adsense-",
						"/adsense.",
						"/adsense/*",
						"/adsense23.",
						"/adsense24.",
						"/adsense?",
						"/adsense_",
						"/adsensegb.",
						"/adserve.",
						"/adserve/*",
						"/adserver.",
						"/adserver/*",
						"/adserver2.",
						"/adserver2/*",
						"/adserver?",
						"/adserver_",
						"/adserversolutions/*",
						"/adservice",
						"/adserving.",
						"/adsfac.",
						"/adsfetch.",
						"/adsfile.",
						"/adsfolder/*",
						"/adshandler.",
						"/adsheader.",
						"/adshow?",
						"/adsiframe/*",
						"/adsign.",
						"/adsimage/*",
						"/adsinclude.",
						"/adsinsert.",
						"/adsky.php",
						"/adslots.",
						"/adslug-",
						"/adslug_",
						"/adsmanagement/*",
						"/adsmanager/*",
						"/adsnew.",
						"/adsonar.",
						"/adsopenx/*",
						"/adspace.",
						"/adsponsor.",
						"/adspro/*",
						"/adsremote.",
						"/adsreporting/*",
						"/adsrich.",
						"/adsrule.",
						"/adssrv.",
						"/adstemplate/*",
						"/adstorage.",
						"/adstracking.",
						"/adstream.",
						"/adstream_",
						"/adswap.",
						"/adswap/*",
						"/adsx728.",
						"/adsync/*",
						"/adsyndication.",
						"/adsystem/*",
						"/adtago.",
						"/adtags.",
						"/adtags/*",
						"/adtagtc.",
						"/adtagtranslator.",
						"/adtech.",
						"/adtech/*",
						"/adtech;",
						"/adtech_",
						"/adtext.",
						"/adtext4.",
						"/adtext_",
						"/adtology.",
						"/adtonomy.",
						"/adtop.do",
						"/adtop.js",
						"/adtrack/*",
						"/adtraff.",
						"/adtype.",
						"/adunit.",
						"/adunits/*",
						"/adv/ads/*",
						"/advert-",
						"/advert.",
						"/advert/*",
						"/advert?",
						"/advert_",
						"/advertise-",
						"/advertise.",
						"/advertise/*",
						"/advertisement-",
						"/advertisement.",
						"/advertisement/*",
						"/advertisement_",
						"/advertisementheader.",
						"/advertisements.",
						"/advertisements/*",
						"/advertisementview/*",
						"/advertising.",
						"/advertising/*",
						"/advertising2.",
						"/advertising_",
						"/advertisingmanual.",
						"/advertisingmodule.",
						"/advertisment.",
						"/advertize_",
						"/advertorials/*",
						"/advertpro/*",
						"/adverts.",
						"/adverts/*",
						"/adverts_",
						"/adview.",
						"/adviewer.",
						"/advision.",
						"/advpartnerinit.",
						"/adworks.",
						"/adworks/*",
						"/adwrapper/*",
						"/adwrapperiframe.",
						"/aff_frame.",
						"/affad?q=",
						"/affads/*",
						"/affilatebanner.",
						"/affiliate*.php?",
						"/affiliate/banners/*",
						"/affiliate_banners/*",
						"/affiliatebanner/*",
						"/affiliatebanners/*",
						"/affiliateimages/*",
						"/affiliates.*.aspx?",
						"/affiliates/banner",
						"/affiliatewiz/*",
						"/affiliationcash.",
						"/afimages.",
						"/afr.php?",
						"/ajax/ads/*",
						"/ajrotator/*",
						"/ajs.php?",
						"/ak/ads/*",
						"/am/ads.",
						"/annonser/*",
						"/api/ads/*",
						"/app/ads.js",
						"/article_ad.",
						"/as3adplayer.",
						"/aseadnshow.",
						"/audioads/*",
						"/auditudeadunit.",
						"/auto_ad_",
						"/ban_m.php?",
						"/banimpress.",
						"/banman.asp?",
						"/banman/*",
						"/banner/affiliate/*",
						"/banner_ad.",
						"/banner_ad_",
						"/banner_ads.",
						"/banner_ads_",
						"/banner_adv/*",
						"/banner_control.php?",
						"/banner_db.php?",
						"/banner_file.php?",
						"/banner_js.*?",
						"/banner_management/*",
						"/banner_skyscraper.",
						"/bannerad.",
						"/bannerads-",
						"/bannerads/*",
						"/banneradviva.",
						"/bannercode.php",
						"/bannerconduit.swf?",
						"/bannerfarm/*",
						"/bannerframe.*?",
						"/bannerframeopenads.",
						"/bannerframeopenads_",
						"/bannermanager/*",
						"/bannermedia/*",
						"/bannerrotation.",
						"/banners.*&iframe=",
						"/banners/affiliate/*",
						"/banners_rotation.",
						"/bannerscript/*",
						"/bannerserver/*",
						"/bannersyndication.",
						"/bannerview.*?",
						"/bannery/*?banner=",
						"/bar-ad.",
						"/baselinead.",
						"/basic/ad/*",
						"/behaviorads/*",
						"/bg_ads_",
						"/bigboxad.",
						"/bkgrndads/*",
						"/blogad_",
						"/blogads.",
						"/blogads3/*",
						"/bmndoubleclickad.",
						"/bnrsrv.*?",
						"/bodyads/*",
						"/boomad.",
						"/bottomad/*",
						"/butler.php?type=",
						"/buttonads.",
						"/buyclicks/*",
						"/bw_adsys.",
						"/bytemark_ad.",
						"/cashad.",
						"/cashad2.",
						"/central/ads/*",
						"/cgi-bin/ads/*",
						"/chitika-ad?",
						"/circads.",
						"/cms/js/ad_",
						"/cnnslads.",
						"/cnwk.1d/ads/*",
						"/commercial_horizontal.",
						"/commercial_top.",
						"/commercials/*",
						"/common/ad/*",
						"/common/ads/*",
						"/companion_ads.",
						"/contentad",
						"/contextad.",
						"/controller/ads/*",
						"/corner_ads/*",
						"/country_ad.",
						"/cpxads.",
						"/cs/ads/*",
						"/ctamlive160x160.",
						"/customad.",
						"/cvs/ads/*",
						"/dart_ads/*",
						"/dartads.",
						"/dc_ads.",
						"/dcloadads/*",
						"/de/ads/*",
						"/defer_ads.",
						"/deferads.",
						"/deliver.nmi?",
						"/deliverad/*",
						"/deliverjs.nmi?",
						"/delivery/ag.php",
						"/descpopup.js",
						"/direct_ads.",
						"/directads.",
						"/display_ads.",
						"/displayad.",
						"/displayads/*",
						"/dj_ad.js",
						"/dne_ad.",
						"/dnsads.html?",
						"/doors/ads/*",
						"/doubleclick.phpi?",
						"/doubleclick/iframe.",
						"/doubleclick_ads.",
						"/doubleclick_ads/*",
						"/doubleclickcontainer.",
						"/doubleclicktag.",
						"/download/ad.",
						"/download/ad/*",
						"/drawad.php?",
						"/dsg/bnn/*",
						"/dt.cms.ad.page.",
						"/dyn_banner.",
						"/dyn_banners_",
						"/dynamic/ads/*",
						"/dynamicad?",
						"/dynamiccsad?",
						"/dynamicvideoad?",
						"/dynanews/ad-",
						"/dynbanner/flash/*",
						"/ebay_ads/*",
						"/emailads/*",
						"/emediatead.",
						"/external/ad.",
						"/external_ads.",
						"/eyewondermanagement.",
						"/fastclick160.",
						"/fastclick728.",
						"/fatads.",
						"/featuredadshome.",
						"/files/ads/*",
						"/fimserve.",
						"/flash/ad_",
						"/flash/ads/*",
						"/flashads.",
						"/flashads/*",
						"/footad-",
						"/footer_ad_",
						"/fuseads/*",
						"/gafsads?",
						"/gamersad.",
						"/genericrichmediabannerad/*",
						"/geo-ads_",
						"/get-ad.",
						"/getad.php?",
						"/getad.php",
						"/getad?n=",
						"/getadframe.",
						"/getadvertimageservlet?",
						"/getarticleadvertimageservlet?",
						"/getbanner.cfm?",
						"/getsponslinks.",
						"/getsponslinksauto.",
						"/getvdopiaads.",
						"/getvideoad.",
						"/gexternalad.",
						"/gfx/ads/*",
						"/glam_ads.",
						"/google-ad?",
						"/google-ads.",
						"/google-adsense-",
						"/google-adsense.",
						"/google_ad_",
						"/google_ads.",
						"/google_ads/*",
						"/google_adsense.",
						"/google_adsense_",
						"/google_afc.",
						"/googlead-",
						"/googlead.",
						"/googleadhtml/*",
						"/googleads-",
						"/googleads.html",
						"/googleads.js",
						"/googleads2.",
						"/googleads_",
						"/googleadsense.",
						"/graphics/ad_",
						"/gt6skyadtop.",
						"/headvert.",
						"/hm_ad.php",
						"/homepage_ads/*",
						"/housead/*",
						"/houseads.",
						"/houseads/*",
						"/houseads?",
						"/html.ng/*",
						"/htmlads/*",
						"/httpads/*",
						"/icon_ad.",
						"/iframe-ads/*",
						"/iframe/ad/*",
						"/iframe_ad.",
						"/iframe_ads/*",
						"/iframe_ads?",
						"/iframe_chitika_",
						"/iframead.",
						"/iframeadsensewrapper.",
						"/im-ad/im-",
						"/images/ad-",
						"/images/ad/*",
						"/images/ad1.",
						"/images/ad125.",
						"/images/ad2.",
						"/images/ad4.",
						"/images/ad5.",
						"/images/ad_",
						"/images/ads-",
						"/images/ads/*",
						"/images/ads_",
						"/images/aff-",
						"/images/gads_",
						"/img/ad_",
						"/img/ads/*",
						"/img/sponsor/*",
						"/imgs/ads/*",
						"/inc/ads/*",
						"/include/ads/*",
						"/includes/ad_",
						"/includes/ads/*",
						"/incmpuad.",
						"/index_ads.",
						"/inline_ad.",
						"/innerads.",
						"/insertads.",
						"/instreamad/*",
						"/interad.",
						"/intextads.",
						"/invideoad.",
						"/inx-ad.",
						"/ipadad.",
						"/irc_ad_",
						"/ireel/ad*.jpg",
						"/iwadsense.",
						"/j/ads.js",
						"/jivoxadplayer.",
						"/jlist-affiliates/*",
						"/js.ng/c",
						"/js.ng/s",
						"/js/ad.js",
						"/js/ad_eo_",
						"/js/ads.",
						"/js/ads/*",
						"/js/ads_",
						"/jsadscripts/*",
						"/jsfiles/ads/*",
						"/keyade.js",
						"/kredit-ad.",
						"/label-advertisement.",
						"/layer-ad.",
						"/layer-ads.",
						"/layerads_",
						"/layout/ads/*",
						"/lbl_ad.",
						"/leader_ad.",
						"/leftad_",
						"/links_sponsored_",
						"/linkshare/*",
						"/liveads.",
						"/loadad.aspx?",
						"/loadadwiz.",
						"/local_ads_",
						"/lsadunit.",
						"/mac-ad?",
						"/magic-ads/*",
						"/main/ad_",
						"/mainad.",
						"/mbn_ad.",
						"/media/ads/*",
						"/megaad.",
						"/metaadserver/*",
						"/mini_ads.",
						"/misc/ad-",
						"/miva_ads.",
						"/mnetorfad.js",
						"/mobilephonesad/*",
						"/mod_ad/*",
						"/modules/ad_",
						"/modules/ads/*",
						"/mpumessage.",
						"/mstextad?",
						"/mtvi_ads_",
						"/mylayer-ad/*",
						"/neoads.",
						"/new/ad/*",
						"/newrightcolad.",
						"/news_ad.",
						"/newtopmsgad.",
						"/nextad/*",
						"/oasadframe.",
						"/oascentral.$~object_subrequest",
						"/oasdefault/*",
						"/oasisi-*.php?",
						"/oasisi.php?",
						"/oiopub-direct/*",
						"/openads-",
						"/openads.",
						"/openads/*",
						"/openads2/*",
						"/openx/www/*",
						"/openx_fl.",
						"/other/ads/*",
						"/overlay_ad_",
						"/ovt_show.asp?",
						"/p2/ads/*",
						"/page-ads.",
						"/pagead/ads?",
						"/pagead/imgad?",
						"/pageadimg/*",
						"/pageads/*",
						"/pageear.",
						"/pageear/*",
						"/pageear_",
						"/pagepeel-",
						"/pagepeel.",
						"/pagepeel/*",
						"/pagepeelads.",
						"/paidlisting/*",
						"/partnerbanner.",
						"/partnerbanner/*",
						"/partners/ads/*",
						"/peel.js",
						"/peel1.js",
						"/peelad/*",
						"/peeljs.php",
						"/perfads.",
						"/performancingads/*",
						"/phpads.",
						"/phpads/*",
						"/phpads2/*",
						"/phpadserver/*",
						"/phpadsnew/*",
						"/pictures/ads/*",
						"/pilot_ad.",
						"/pitattoad.",
						"/pix/ad/*",
						"/play/ad/*",
						"/player/ads/*",
						"/pool.ads.",
						"/pop_ad.",
						"/popads.",
						"/popunder.",
						"/position=*/aamsz=",
						"/predictad.",
						"/premierebtnad/*",
						"/premium_ad.",
						"/printad.",
						"/printads/*",
						"/processads.",
						"/promobuttonad.",
						"/promotions/ads.",
						"/promotions/ads?",
						"/protection/ad/*",
						"/pub/ad/*",
						"/pub/ads/*",
						"/public/ad/*",
						"/public/ad?",
						"/publicidad.$~object_subrequest",
						"/publicidad/*",
						"/questions/ads/*",
						"/radopenx?",
						"/railad.",
						"/railads.",
						"/random=*/aamsz=",
						"/realmedia/ads/*",
						"/rec_ad1.",
						"/reclame/*",
						"/refreshads-",
						"/reklam.",
						"/reklama.",
						"/relatedads.",
						"/requestadvertisement.",
						"/requestmyspacead.",
						"/right-ad-",
						"/rightad.",
						"/rightnavadsanswer.",
						"/rotateads.",
						"/rotatingpeels.js",
						"/rsads/c",
						"/rsads/r",
						"/rsads/u",
						"/scanscoutoverlayadrenderer.",
						"/scripts/ad-",
						"/scripts/ad.",
						"/scripts/ads/*",
						"/scripts/clickjs.php",
						"/sd/ad.php",
						"/search/ad/*",
						"/search/ads?",
						"/searchads/*",
						"/secondads.",
						"/secondads_",
						"/services/ads/*",
						"/sevenl_ad.",
						"/share/ads/*",
						"/shared/ads/*",
						"/show_ad.",
						"/show_ad_",
						"/show_ads.js?",
						"/show_ads_",
						"/showad.",
						"/showad/*",
						"/showads.",
						"/showadvertising.",
						"/showflashad.",
						"/showlayer.",
						"/side-ads-",
						"/sideads/*",
						"/sideads",
						"/sidebar_ad.",
						"/sidebarad/*",
						"/sidecol_ad.",
						"/silver/ads/*",
						"/siteads.",
						"/siteads/*",
						"/siteafs.txt?",
						"/sites/ad_",
						"/skyad.php",
						"/skyadjs/*",
						"/skyframeopenads.",
						"/skyframeopenads_",
						"/slideinad.",
						"/small_ad.",
						"/smartad.",
						"/smartads.",
						"/smb/ads/*",
						"/socialads.",
						"/socialads/*",
						"/spc.php?",
						"/spcjs.php?",
						"/special-ads/*",
						"/special_ads/*",
						"/spo_show.asp?",
						"/sponsimages/*",
						"/sponslink_",
						"/sponsor-ad",
						"/sponsor_images/*",
						"/sponsorad.",
						"/sponsoradds/*",
						"/sponsorads/*",
						"/sponsored_ad.",
						"/sponsored_links_",
						"/sponsored_text.",
						"/sponsored_top.",
						"/sponsoredlinks.",
						"/sponsoredlinks/*",
						"/sponsors/*",
						"/sponsors_box.",
						"/square-ads/*",
						"/squaread.",
						"/srv/ad/*",
						"/static/ad_",
						"/static/ads/*",
						"/stickyad.",
						"/storage/ads/*",
						"/story_ad.",
						"/swf/ad-",
						"/synad2.",
						"/system/ad/*",
						"/systemad.",
						"/systemad_",
						"/templates/_ads/*",
						"/testingad.",
						"/textad.",
						"/textad?",
						"/textads/*",
						"/textads_",
						"/thirdparty/ad/*",
						"/tii_ads.",
						"/tikilink?",
						"/tmo/ads/*",
						"/tmobilead.",
						"/toigoogleads.",
						"/toolkitads.",
						"/top-ad-",
						"/top_ad.",
						"/top_ads/*",
						"/top_ads_",
						"/trackads/*",
						"/tracked_ad.",
						"/tradead_",
						"/tribalad.",
						"/ttz_ad.",
						"/tx_macinabanners/*",
						"/update_ads/*",
						"/upload/ads/*",
						"/uploads/ads/*",
						"/valueclick.",
						"/vclkads.",
						"/video/ads/*",
						"/video_ad.",
						"/video_ad_",
						"/videoad.",
						"/videoads.",
						"/videoads/*",
						"/videowall-ad.",
						"/view/banner/*/zone?zid=",
						"/vr-ad.png",
						"/vtextads.",
						"/webad?a",
						"/webadimg/*",
						"/webads.",
						"/webads_",
						"/webadverts/*",
						"/wipeads/*",
						"/wp-content/ads/*",
						"/wp-srv/ad/*",
						"/wpads/iframe.",
						"/writelayerad.",
						"/www/delivery/*",
						"/xmladparser.",
						"/yahoo-ads/*",
						"/your-ad.",
						"/ysmads.",
						"/zanox/banner/*",
						"/zanox_ad/*",
						"8080/ads/",
						";adsense_",
						";iframeid=ad_",
						"=adtech_",
						"=advert/",
						"=advertorial&",
						"?ad_ids=*&referer=",
						"?ad_type=",
						"?ad_width=",
						"?adarea=",
						"?adclass=",
						"?adpage=",
						"?adtype=",
						"?advertising=",
						"?advtile=",
						"?advurl=",
						"?file=ads&",
						"?getad=&$~object_subrequest",
						"_728.htm",
						"_ad.aspx",
						"_ad.php?",
						"_ad_big.",
						"_ad_code.",
						"_ad_courier.",
						"_ad_homepage.",
						"_ad_label.",
						"_adagency/",
						"_adcall_",
						"_adpage=",
						"_adplugin.",
						"_ads.php?",
						"_ads_index_",
						"_ads_reporting.",
						"_ads_single_",
						"_adserve/",
						"_adspaces/",
						"_adtitle.",
						"_advert.",
						"_advert1.",
						"_advertise.",
						"_advertisehere.",
						"_advertisement.",
						"_advertisement_",
						"_advertising/",
						"_advertisment.",
						"_advertorials/",
						"_adwrap.",
						"_afs_ads.",
						"_assets/ads/",
						"_banner_adv_",
						"_bannerad.",
						"_blogads.",
						"_box_ads/",
						"_buttonad.",
						"_companionad.",
						"_custom_ad.",
						"_custom_ad_",
						"_displaytopads.",
						"_dynamicads/",
						"_fd_adbg1a.",
						"_fd_adbg2.",
						"_fd_adbg2a.",
						"_fd_adtop.",
						"_gads_bottom.",
						"_gads_footer.",
						"_gads_top.",
						"_images/ads/",
						"_overlay_ad.",
						"_player_ads_",
						"_request_ad.",
						"_temp/ad_",
						"_topad.php",
						"_videoad.",
						"a/adx.js",
						"d/adx.js",
						"e/adx.js",
						"e/js.ng/",
						"e/js_ng/",
						"k/ads.pl",
						"m/adbox/",
						"m/ads.pl",
						"m/adx.js",
						"m/js.ng/",
						"m/topads",
						"n/adx.js",
						"r/ad.htm",
						"s/adx.js",
						"t/adx.js",
						"u/adx.js",
						"z/adx.js",


						"http://a.ads.",
						"http://ad-uk.",
						"http://ad0.",
						"http://ad1.",
						"http://ad2.",
						"http://ad3.",
						"http://ad4.",
						"http://ad5.",
						"http://ad6.",
						"http://ad7.",
						"http://adimg.",
						"http://adnet.",
						"http://ads.",
						"http://ads0.",
						"http://ads1.",
						"http://ads18.",
						"http://ads2.",
						"http://ads3.",
						"http://ads4.",
						"http://ads5.",
						"http://adservingit.",
						"http://adseu.",
						"http://adsrv.",
						"http://adsvr.",
						"http://adsys.",
						"http://adver.",
						"http://advertiser.",
						"http://bwp.*/search",
						"http://feeds.*/~a/",
						"http://getad.",
						"http://jazad.",
						"http://openx.",
						"http://pubad.",
						"http://rss.*/~a/",
						"http://synad.",
						"http://u-ads.",
						"http://wrapper.*/a?",
						"da.feedsportal.com/r/",
						"http://feeds.feedburner.*bild?",
						"feedflare",
						"http://feeds.feedburner.*bild/",
						"http://rss.feedsportal.*/mf.gif"
					];



    $.dcmsAdblocker = {

        // Public methods
        init: function(selector) {
			var search = $.dcmsAdblocker.wordRegexp(adFilters);

			links = $(selector).find('*');
			links.each(function(){
				href = $(this).attr('href');
				if ( search.test( href ) )
				{
					$(this).replaceWith(adReplace);
				}
				src = $(this).attr('src');
				if ( search.test( src ) )
				{
					$(this).replaceWith(adReplace);
				}
				id = $(this).attr('id');
				if ( search.test( id ) )
				{
					$(this).replaceWith(adReplace);
				}
				aclass = $(this).attr('class');
				if ( search.test( aclass ) )
				{
					$(this).replaceWith(adReplace);
				}
			});
        },

		wordRegexp: function(words) {
			var str = words.join("|");
			str = str.replace(/\$/g, '');
			str = str.replace(/\?/g, '\\?');
			str = str.replace(/\$/g, '\\$');
			str = str.replace(/\+/g, '\\+');
			var RegExpStr = "(" + str +")";
			return new RegExp(RegExpStr, "gim");
		}
    }

    // Shortuct functions
    dcmsAdblocker = function(selector) {
        $.dcmsAdblocker.init(selector);
    };

})(jQuery);