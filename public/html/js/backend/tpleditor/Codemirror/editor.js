...
var Editor = (function () {
  ...
  function Editor(options) {
    ...
    this.updateErrorLines = options.updateErrorLines; 
  ...
  Editor.prototype = {
    ...
    keyDown: function (event) {
      ...
      else if ( (event.shiftKey && code == 55) || code == 46)
      {
      	if ( this.updateErrorLines != null )
      	{
      		clearTimeout(top.validateSyntaxTimer);
      ...
      // Only in Firefox is the default behavior for PgUp/PgDn correct.
        ...
        else if (code == 86 && event.shiftKey && this.options.previewFunction) { // P
          this.options.previewFunction();
          event.stop();
        }
        else if (code == 68 && event.shiftKey && this.options.deleteFunction) { // D
          this.options.deleteFunction();
          event.stop();
        }
        else if (code == 82 && event.shiftKey && this.options.renameFunction) { // R
          this.options.renameFunction();
          event.stop();
        }
        else if (code == 77 && event.shiftKey && this.options.modeFunction) { // M
          this.options.modeFunction();
          event.stop();
        }
        else if (code == 13 && this.options.maximizeFunction) { // ctrl+enter
          this.options.maximizeFunction();
          event.stop();
        }
    ...
    keyUp: function (event) {
    	...
    	var code = event.keyCode;
      if ((event.shiftKey && code == 55) || code == 13 || code == 8 || code == 46) {
    ...
    highlight: function (from, target, cleanLines, maxBacktrack) {
      ...
      forEach(parsed, function (token) {
        ...
        if (parsed.context) part.parserContext = parsed.context();
        if (token.value == "\n") {
