        ...
        Plugin = (function() {
            ...
            Plugin.prototype.process = function(file) {
                ...
                send = function(chunk, url) {
                    ...
                    if (_this.opts.stop)
                    {
                        if (_this.opts.onCancel)
                        {
                            _this.opts.onCancel(file);
                        }
                    ...
                    hash = (_this.hash(file.name) + file.size).toString();
                    if (_this.opts.beforeEach(file) === false) {
                        _this.opts.error(_this.errors.uploadHalted, file);
                        next_file();
                        return false;
                    }
                    fd = new FormData;
                ...
                if (this.opts.max_file_size > 0 && file.size > 1048576 * this.opts.max_file_size) {
                    this.opts.error(this.errors.tooLarge, file);
                    next_file();
                    return false;
                }
