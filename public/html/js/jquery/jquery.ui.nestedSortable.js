    ...
    $.widget("ui.nestedSortable", $.extend({}, $.ui.sortable.prototype, {
        ...
        _uiHash: function(inst) {
            ...
            return {
                ...
                direction: self.direction == 'down' ? 'up' : 'down',
                leftright: self.before,
                VerticalDirection: self.VerticalDirection,
                HorizontalDirection: self.HorizontalDirection,
                move: self.move,
        ...
        _mouseDrag: function(event) {
            ...
            this.VerticalDirection = this._getDragVerticalDirection(item);
            this.HorizontalDirection = this._getDragHorizontalDirection(item);
        ...
        serialize: function(o) {
            ...
            $(items).each(function() {
                ...
                if (res)
                    str.push((o.key || res[1] + '[' + (o.key && o.expression ? res[1] : res[2]) + ']') + '=' + (pid ? (o.key && o.expression ? pid[1] : pid[2]) : 'root'));
            });
        ...
        toArray: function(o) {
            ...
            var root = $(this.element).children('li:first');
            var rootLeft = $(root).attr('rel').split('-');
            var rootRight = rootLeft[1];
            rootLeft = rootLeft[0];
            var diff = 2;

            ret.push({"item_id": 'root', "parentid": 'none', "level": sDepth, "lft": rootLeft, "rgt": ($('li', this.element).length + 1) * 2});

            $(this.element).children('li').each(function() {
                left = _recursiveArray($(this), sDepth + 1, left, rootRight);
            });

            return ret;

            function _recursiveArray(item, depth, left, rootRight) {
                ...
                if ($(item).children(o.listType).children('li').length > 0) {
                    ...
                    $(item).children(o.listType).children('li').each(function() {
                        right = _recursiveArray($(this), depth, right, rootRight);
                    });
                    depth--;
                }
                ...
                ret.push({"item_id": id[2], "parentid": pid, "level": depth, "lft": left, "rgt": right + diff});
