		...
		_fn : { 
			...
			open_node	: function (obj, callback, skip_animation) {
				...
				if(!this._is_loaded(obj)) {
					obj.find("a").addClass("jstree-loading");
					this.load_node(obj, function () { t.open_node(obj, callback, skip_animation); }, callback);
				}
				else {
					if(s) { obj.children("ul").css("display","none"); }
					obj.removeClass("jstree-closed").addClass("jstree-open").find("a").removeClass("jstree-loading");
			...
			move_node : function (obj, ref, position, is_copy, is_prepared, skip_check) {
				...
				if(obj.or.length) { obj.or.before(o); }
					...
					if(!obj.np.find("ul").length) { $("<ul>").appendTo(obj.np); }
					obj.np.find("ul:eq(0)").append(o); 
				}
				...
				try { 
					...
					if(!obj.op.find("> ul > li").length) {
						obj.op.removeClass("jstree-open jstree-closed").addClass("jstree-leaf").find("ul").remove();
					}
				} catch (e) { }
	...
	$.jstree.plugin("ui", {
		...
		_fn : { 
			...
			hover_node : function (obj) {
				...
				this.data.ui.hovered = obj.find("a").addClass("jstree-hovered").parent();
				this.__callback({ "obj" : obj });
			},
			dehover_node : function () {
				var obj = this.data.ui.hovered, p;
				if(!obj || !obj.length) { return false; }
				p = obj.find("a").removeClass("jstree-hovered").parent();
			...
			select_node : function (obj, check, e) {
				...
				if(proceed && !is_selected) {
					obj.find("a").addClass("jstree-clicked");
			...
			deselect_node : function (obj) {
				...
				if(this.is_selected(obj)) {
					obj.find("a").removeClass("jstree-clicked");
...
(function ($) {
	...
	$.jstree.plugin("dnd", {
		...
		_fn : {
			...
			dnd_show : function () {
				...
				switch(r) {
					case "before":
						m.css({ "left" : pos + "px", "top" : (this.data.dnd.off.top - 8) + "px" }).show();
						break;
					case "after":
						m.css({ "left" : pos + "px", "top" : (this.data.dnd.off.top + this.data.core.li_height - 8) + "px" }).show();
						break;
					case "inside":
	...
	$(function() {
		var css_string = '' + 
			...
			'#jstree-marker { padding:0; margin:0; line-height:12px; font-size:1px; overflow:hidden; height:12px; width:8px; position:absolute; top:-30px; z-index:999999; background-repeat:no-repeat; display:none; background-color:silver; } ';
		$.vakata.css.add_sheet({ str : css_string });
...
(function ($) {
	$.vakata.context = {
		...
		parse	: function (s, is_callback) {
			...
			str += "<ul class='dcmscontextmenu'>";
			$.each(s, function (i, val) {
				if(!val) { return true; }
				$.vakata.context.func[i] = val.action;
				if(!was_sep && val.separator_before) {
					str += "<li class='vakata-separator vakata-separator-before'><span></span></li>";
				}
				was_sep = false;
				...
				*/
				...
				str += "<a href='#' rel='" + i + "'><span ";
				if(val.icon && val.icon.indexOf("/") === -1) { str += " class='" + val.icon + "' "; }
				if(val.icon && val.icon.indexOf("/") !== -1) { str += " style='background:url(" + val.icon + ") center center no-repeat!important;' "; }
				str += ">&nbsp;</span>";
	...
	$(function () {
		var css_string = '' + 
			...
			'#vakata-contextmenu ul, #vakata-contextmenu li { list-style-type:none; display:block; } ' + 
			'#vakata-contextmenu li { position:relative; } ' + 
			'#vakata-contextmenu li a { display:block; text-decoration:none; } ' + 
			'#vakata-contextmenu li a span { float:left;display:block!important; width:16px; height:16px; text-decoration:none; margin-right:8px; } ' + 
			'#vakata-contextmenu li a:hover, #vakata-contextmenu li.vakata-hover > a { left: 0!important; } ' + 
		...
		if(typeof $.hotkeys !== "undefined") {
			$(document)
				...
				.bind("keydown", "space", function (e) { 
					$.vakata.context.cnt.find(".vakata-hover").last().find("a").click();
					e.preventDefault();
				});
		}
	});
	...
	$.jstree.plugin("contextmenu", {
		...
		_fn : {
			show_contextmenu : function (obj, x, y) {
				...
				if(this.data.themes) {
				 $.vakata.context.cnt.attr("class", /* "jstree-" + this.data.themes.theme + "-context */ "dcmscontextmenu"); 
				...
				.bind("init.jstree", $.proxy(function () { 
						...
						$.each(types, function (i, tp) {
							...
							if( tp.icon.image || tp.icon.position) {
								if(i == "default")	{ icons_css += '.jstree-' + _this.get_index() + ' a ins.jstree-icon { '; }
								else				{ icons_css += '.jstree-' + _this.get_index() + ' li[' + attr + '=' + i + '] a ins.jstree-icon { '; }
		...
		_fn : {
			...
			_check : function (rule, obj, opts) {
				...
				if(rule === "max_depth" && obj !== -1 && opts !== false && s.max_depth !== -2 && v !== 0) {
					...
					this._get_node(obj).find("a:eq(0)").parentsUntil(".jstree","li").each(function (i) {
		...
		__init : function () {
			...
			this.get_container()
				...
				.bind("select_node.jstree", $.proxy(function (e, data) {
						data.rslt.obj.find("a").addClass(s.item_a);
					}, this))
	...
	$.jstree.plugin("unique", {
						...
						if(data.func == "move_node") {
							...
							if(data.args[4] === true) {
								if(data.args[0].o && data.args[0].o.length) {
									data.args[0].o.find("a").each(function () { nms.push($(this).text().replace(/^\s+/g,"")); });
		...
		_fn : { 
			_check_unique : function (nms, p) {
				var cnms = [];
				p.find("a").each(function () { cnms.push($(this).text().replace(/^\s+/g,"")); });
			...
			check_move : function () {
				...
				if(p.o && p.o.length) {
					p.o.find("a").each(function () { nms.push($(this).text().replace(/^\s+/g,"")); });
