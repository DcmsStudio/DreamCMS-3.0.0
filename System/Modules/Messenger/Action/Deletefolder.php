 ...
 */
class Messenger_Action_Deletefolder extends Controller_Abstract
{
	...
	public function execute ()
	{
		...
		if ( $folder > 3 )
		{
			$this->db->query("DELETE FROM %tp%messages_folders WHERE id = ? AND userid = ?", $folder, User::getUserId());
			$this->db->query("UPDATE %tp%messages SET folder = 3 WHERE folder = ? AND touser = ?", $folder, User::getUserId());
		}
