...
class Firewall_Config_Base {
	...
	public static function getPermissions ( $getBackend = false )
	{
		...
		else
		{
			return array (
				...
				'access-items' => array (
					'index'    => array (
						trans('Firewall einsehen'),
						0
					),
					'delete'  => array (
						trans('Firewall-Logs löschen'),
						0
					)
				)
			);
	...
	{
		$menu = array (
			'label' => trans('Firewall'),
			'description' => null,
			'icon'        => null,
			'action'      => ''
		);

		Menu::addMenuItem('system', 'firewall', $menu);
	}
	...
	public static function getModulDefinition ()
	{
		...
		return array (
			'dockurl'       => 'admin.php?adm=firewall',
			'modulelabel'   => trans('Firewall'),
			'allowmetadata' => false,
			'version'       => '0.1',
			'metatables'    => array ()
		);
