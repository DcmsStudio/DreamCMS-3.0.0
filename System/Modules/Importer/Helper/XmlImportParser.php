 ...
 */
class Importer_Helper_XmlImportParser
{
	...
	public function __construct ( $xml, $rootNodeXPath, $cachedTemplate, $isURL = false )
	{
		...
		catch ( Exception $e )
		{
			...
			{
				throw new Importer_Helper_XmlImportException($e->getMessage());
			}
		}
	...
	public function parse ( $records = array () )
	{
		...
		empty($records) or is_array($records) or $records = array (
			$records
		);
		...
		if ( $rootNodes === false )
		{
			throw new Importer_Helper_XmlImportException('Invalid root node XPath \'' . $this->rootNodeXPath . '\' specified');
		}
		for ( $i = 0; $i < count($rootNodes); $i++ )
		{
			if ( empty($records) or in_array($i + 1, $records) )
			{
				...
				$template  = new Importer_Helper_XmlImportTemplate($rootNode, $this->cachedTemplate);
	 ...
	 * @return Importer_Helper_XmlImportParser
	 */
	public static function factory ( $xml, $rootNodeXPath, $template, &$file = null )
	{

		$scanner       = new Importer_Helper_XmlImportTemplateScanner();
		$tokens        = $scanner->scan(new Importer_Helper_XmlImportStringReader($template));
		$t_parser      = new Importer_Helper_XmlImportTemplateParser($tokens);
		$tree          = $t_parser->parse();
		$codegenerator = new Importer_Helper_XmlImportTemplateCodeGenerator($tree);
