 ...
 */
class Importer_Helper_XmlImportTemplateParser
{
	...
	public function parse ()
	{
		...
		{
			throw new Importer_Helper_XmlImportException("Unexpected end of template.");
		}
	...
	private function parseSequence ()
	{
		...
		{
			throw new Importer_Helper_XmlImportException("Reached end of template but statement sequence expected");
		}
	...
	private function parseStatement ()
	{
		...
		if ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_TEXT )
		{
			return new XmlImportAstText($this->tokens[ ++$this->index ]->getValue());
		}
		elseif ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_PRINT )
		{
			$this->index++;

			return new XmlImportAstPrint($this->parseExpression());
		}
		elseif ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_WITH )
		{
			return $this->parseWith();
		}
		elseif ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_FOREACH )
		{
			return $this->parseForeach();
		}
		elseif ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_MATH )
		{
			return new XmlImportAstPrint($this->parseExpression());
		}
		elseif ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_SPINTAX )
		{
			return new XmlImportAstPrint($this->parseExpression());
		}
		elseif ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_IF )
		{
			return $this->parseIf();
		}
		elseif ( $this->clauseStack[ count($this->clauseStack) - 1 ] == Importer_Helper_XmlImportToken::KIND_ENDIF && in_array($this->tokens[ $this->index + 1 ]->getKind(), array (
		                                                                                                                                                                           Importer_Helper_XmlImportToken::KIND_ELSE,
		                                                                                                                                                                           Importer_Helper_XmlImportToken::KIND_ELSEIF
		                                                                                                                                                                     ))
		)
		{
			if ( $this->elseAllowed )
			{
				if ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_ELSE )
				{
			...
			{
				throw new Importer_Helper_XmlImportException("ELSEIF or ELSE is not allowed again after ELSE");
			}
		...
		{
			throw new Importer_Helper_XmlImportException("Unexpected token {$this->tokens[ $this->index + 1 ]->getKind()}, statement was expected.");
		}
	}
	...
	private function parseExpression ()
	{
		...
		if ( $this->index + 1 == count($this->tokens) )
		{
			throw new Importer_Helper_XmlImportException("Reached end of template but expression was expected");
		}
		if ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_FUNCTION )
		{
		...
		elseif ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_XPATH )
		{
		...
		elseif ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_STRING || $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_OPERATION )
		{
			return new XmlImportAstString($this->tokens[ ++$this->index ]->getValue());
		}
		elseif ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_INT )
		{
			return new XmlImportAstInteger($this->tokens[ ++$this->index ]->getValue());
		}
		elseif ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_FLOAT )
		{
			return new XmlImportAstFloat($this->tokens[ ++$this->index ]->getValue());
		}
		else
		{
			throw new Importer_Helper_XmlImportException("Unexpected token " . $this->tokens[ $this->index + 1 ]->getKind());
		}
	}
	...
	private function parseFunction ()
	{
		...
		if ( $this->tokens[ $this->index + 1 ]->getKind() != Importer_Helper_XmlImportToken::KIND_OPEN )
		{
			throw new Importer_Helper_XmlImportException("Open brace expected instead of " . $this->tokens[ $this->index + 1 ]->getKind());
		}
		$this->index++;
		if ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_CLOSE )
		{
			$this->index++;
		...
		else
		{
			while ( $this->index < count($this->tokens) - 2 )
			{
				...
				elseif ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_COMMA )
				{
					$this->index++;
				}
				else
				{
					throw new Importer_Helper_XmlImportException("Comma or closing brace expected instead of " . $this->tokens[ $this->index + 1 ]->getKind());
				}
			}
			throw new Importer_Helper_XmlImportException("Unexpected end of {$function->getName()} function argument list");
		}
	}
	...
	private function parseMath ()
	{
		...
		else
		{
			...
			throw new Importer_Helper_XmlImportException("Unexpected end of MATH argument list");
		}
	}
	...
	private function parseXPathDependant ()
	{
		...
		if ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_XPATH )
		{
			$xpath = new XmlImportAstXPath($this->tokens[ ++$this->index ]->getValue());
			$this->sequenceStack[ count($this->sequenceStack) - 1 ]->addVariable($xpath);
		}
		...
		{
			throw new Importer_Helper_XmlImportException("XPath expression expected instead of " . $this->tokens[ $this->index + 1 ]->getKind());
		}
		if ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_CLOSE )
		{
			$this->index++;
		...
		{
			throw new Importer_Helper_XmlImportException("Close brace expected instead of " . $this->tokens[ $this->index + 1 ]->getKind());
		}
	...
	private function parseWith ()
	{
		...
		//store sequence exit
		array_push($this->clauseStack, Importer_Helper_XmlImportToken::KIND_ENDWITH);
	...
	private function parseForeach ()
	{
		...
		array_push($this->clauseStack, Importer_Helper_XmlImportToken::KIND_ENDFOREACH);
	...
	private function parseIf ()
	{
		...
		if ( $this->index + 1 != count($this->tokens) )
		{
			while ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_ELSEIF )
			{
				$this->index++;
				if ( $this->tokens[ $this->index + 1 ]->getKind() == Importer_Helper_XmlImportToken::KIND_OPEN )
				{
