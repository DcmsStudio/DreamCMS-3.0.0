 ...
 */
class Importer_Helper_XmlImportTemplateCodeGenerator
{
	...
	public function generate ( $filename = null )
	{
		...
		if ( is_null($filename) )
		{
			$filename = tempnam(Importer_Helper_XmlImportConfig::getInstance()->getCacheDirectory(), 'xim');
		}
		if ( !$filename )
		{
	...
	private function generateForStatement ( XmlImportAstStatement $statement )
	{
		...
		else
		{
			...
			{
				$var = '$x' . $this->xmlVariableNumber++;
				$result .= PHP_EOL . 'foreach (' . $this->generateForExpression($statement->getXPath()) . ' as ' . $var . ') :' . PHP_EOL;
				array_push($this->xmlStack, $var);
				$result .= $this->generateForSequence($statement->getBody());
