...
class Importer_Helper_Chuck
{
	...
	public function __construct ( $file, $options = array (), $pointer = 0, $return_with_encoding = true )
	{
		...
		$this->options[ 'chunkSize' ] = (int)$this->options[ 'chunkSize' ];
		...
		$this->options[ 'chunkSize' ] *= 10; // 1mb chuck
	...
	public function read ()
	{
		...
		if ( empty($element) )
		{
			...
			while ( $this->reading && (count($founded_tags) < 500) )
			{
		...
		if ( empty($this->encoding) )
		{
			$this->encoding = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>";
		}

		if ( empty($element) && !empty($founded_tags) )
		{
			...
			if ( !empty($element_counts) )
			{
				...
				foreach ( $element_counts as $tag => $count )
				{
					if ( strpos($tag, ":") === false )
					{
						if ( $count > 1 && empty($this->options[ 'element' ]) )
						{
							$element                    = $tag;
		...
		if ( empty($element) )
		{
			if ( !empty($element_counts) )
			{
				$element                    = array_shift(array_keys($element_counts));
				$this->options[ 'element' ] = $element;
			}
			else
			{
		...
		while ( $this->reading && !@feof($this->handle) )
		{
			...
			$withoutcloseelement = (preg_match("%<" . $element . "\s{1,}[^<]*\/>%i", $tmp, $matches)) ?
				strpos($tmp, $matches[ 0 ]) : false;

			if ( $withoutcloseelement and $checkClose and $closematches[ 0 ][ 0 ][ 1 ] > $withoutcloseelement )
			{
				$checkClose = false;
			}
			...
			if ( !$checkClose )
			{
				$checkClose = (preg_match("%<" . $element . "\s{1,}[^<]*\/>%i", $tmp, $matches)) ?
					strpos($tmp, $matches[ 0 ]) : false;
				...
				{
					$checkClose = (preg_match_all("%<" . $element . "\s{1,}[^<]*\/>%i", $this->readBuffer, $matches)) ?
						strpos($this->readBuffer, $matches[ 0 ][ count($matches[ 0 ]) - 1 ]) : false;
					if ( $checkClose !== false )
					{
						$withoutcloseelement = true;
			...
			else
			{
				...
				while ( !$close_finded )
				{
					if ( $checkDuplicateOpen > 1 && !empty($closematches[ 0 ][ $checkDuplicateOpen - 1 ]) )
					{
