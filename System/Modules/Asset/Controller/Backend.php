 ...
 * @file         Backend.php
 */
class Asset_Controller_Backend extends Controller_Abstract
{
	...
	public function runAction ( Controller $controller )
	{
		...
		if ( $this->getApplication()->getMode() !== Application::BACKEND_MODE )
		{
			...
			if ( substr(REQUEST, -3) !== '.js' && substr(REQUEST, -3) !== '.css' )
			{
				if ( is_file(ROOT_PATH . REQUEST) )
				{
					die(readfile(ROOT_PATH . REQUEST));
				}


				Library::redirect(Settings::get('portalurl') . '/' . $mediaFile);
			}
