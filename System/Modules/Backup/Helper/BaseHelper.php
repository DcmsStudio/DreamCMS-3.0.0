 ...
 * @file         BaseHelper.php
 */
class Backup_Helper_BaseHelper extends Controller_Abstract
{
	 ...
	 * @return array
	 */
	public function getCoreDirectorysAndFiles ()
	{

		$dirs  = Library::getFiles(ROOT_PATH, true);
		$path  = ROOT_PATH;
		$files = array ();

		$regex = '/System|/Vendor|/Templates|/public|/LICENSE|/index\.php|/admin\.php|/setup\.php|/\.htaccess|/public/\.htaccess|/robots\.txt';

		foreach ( $dirs as $r )
		{
			if ( !$r[ 'filename' ] || preg_match('#(([\s\-_]*)dev([\s\-_]*)|\.bak|\.ds_store)#i', $r[ 'filename' ]) || preg_match('#(/backup|/\.tmb|/thumbnails)#i', $r[ 'path' ]) )
			{
				continue;
			}

			if ( preg_match('#(' . $regex . ')#s', $r[ 'path' ]) || preg_match('#(' . $regex . ')#s', $r[ 'filename' ]) )
			{
				$files[ ] = str_replace(ROOT_PATH, '', $r[ 'path' ]) . $r[ 'filename' ];
			}
		}
		unset($dirs);
