...
class CoreTag_News_Link extends Provider_Abstract
{
	...
	public function render($tag)
    {
        ...
        if ($_id > 0)
        {
            ...
            if (isset($data['id']))
            {
                $output .= '<a href="news/item/' . $data['alias'] . ($data['suffix'] ? '.' . $data['suffix'] : '.html') .'">'.$data['title'].'</a>';
            }
        ...
        elseif ($_alias)
        {
            ...
            {
                $transq1 = $this->buildTransWhere('news', 'n.id', 'nt');
                $data = $this->db->query('SELECT n.id, nt.`lang`, nt.alias, nt.suffix, nt.title
                                                            FROM %tp%news AS n
                                                            LEFT JOIN %tp%news_trans AS nt ON(nt.id=n.id)
