 ...
 * @file         Closethread.php
 */
class Addon_Forum_Action_Closethread extends Addon_Forum_Helper_Base
{
	...
	public function execute ()
	{
		...
		if ( $this->isFrontend() )
		{
			...
			if ( !$is_array )
			{
				...
				if ( !$data[ 'ismod' ] || !$data[ 'mod' ][ 'canopenclosethread' ] )
				{
					$this->Page->sendAccessError(trans('Sie besitzen nicht die nötigen Rechte um dieses das Thema zu schließen/öffnen!'));
				}

				$this->model->changeThreadCloseing($thread[ 'threadid' ], $mode);

				$this->model->sync('thread', $threadid);
				$this->model->sync('forum', $thread[ 'forumid' ]);

				Library::log(sprintf('Forum Moderator has ' . ( !$mode ? 'open' : 'close' ) . ' the Thread `%s`', $thread[ 'title' ]));


				if ( IS_AJAX )
				{
					Library::sendJson(true, sprintf(trans('Das Thema `%s` wurde %s'), $thread[ 'title' ], ( !$mode ? trans('geöffnent') : trans('geschlossen') )));
				}
				else
				{
					Library::redirect('/plugin/forum/thread/' . $thread[ 'threadid' ] . '/' . ( Url::makeRw($thread[ 'alias' ], $thread[ 'suffix' ], $thread[ 'title' ]) ) . '?mod=1&threadclosed=' . $mode);
				}
				...
				$this->model->changeThreadCloseing($tmp, $mode);
				...
				if ( IS_AJAX )
				{
					Library::sendJson(true, sprintf(trans('Es wurden %s Themen %s'), count($tmp), ( !$mode ? trans('geöffnent') : trans('geschlossen') ) ));
				}
				else
				{
					die();
