...
$errorcode = $exception->_errorCode ? $exception->_errorCode : $exception->getCode();
$message = $data[ 'sql_message' ] ? $data[ 'sql_message' ] : $data[ 'message' ];
...
$sqlerror = $exception->sqlCode;
$sqlargs = $exception->sqlArgs;
        ...
        $(document).ready(function () {
            ...
            $('#tabs li').click(function () {
                if (!$(this).hasClass('active')) {
                    var lastId = $('#tabs li.active').removeClass('active').attr('rel');
                    $('#' + lastId).hide();
            ...
            var firstDiv = $('div.tabcontent:first');
            var offset = firstDiv.offset();

            var maxHeight = $(window).height() - offset.top - 30;

            $('div.tabcontent').height(maxHeight);

            $('div.tabcontent').find('.input-vars,.executed-sql,.trace-content-container,.trace-list,.trace-content').height(maxHeight - 10);
...
<div id="main">
    <div class="box exception-message-container">
        ...
        <div>
            <div class="exception-message <?php echo strtolower( $errortype ); ?>">
                <?php
                echo Debug::protectPath( $message );
                ?>
            </div>
            ...
            <div class="exception-info <?php echo strtolower( $errortype ); ?>">
                ...
                elseif ( strtolower( $errortype ) == 'compiler' )
                {
                    ?>
                    <dl>
                        <dt>Type:</dt>
                ...
                if ( $file )
                {
                    ...
                    <dl>
                        <dt>File:</dt>
                        <dd><?php echo Debug::protectPath( $file ); ?></dd>
                    </dl>
                <?php
                }
    ...
    <div class="tabs">
        <ul id="tabs">
            <li rel="tab1" class="active">Debug</li>
            <li rel="tab2">Vars</li>
            <?php
            if ( !empty( $compiler_code ) )
            {
                ?>
                <li rel="tab3">Compiler Error</li>
            <?php
            }
            if ( isset( $sqlerror ) && !empty( $sqlerror ) )
            {
                ?>
                <li rel="tab4">Sql Error</li>
            <?php
            }
            ?>
                ...
                <div class="trace-content-container">
                    <div class="trace-list">
                        <ul id="trace-list">
                            ...
                            {
                                echo '<li rel="trace-' . $key . '"' . ( $key == 0 ? ' class="selected"' : '' ) . '>' . Debug::protectPath( $r[ 'file' ] ) . '  [' . $r[ 'line' ] . ']</li>';
                            }
                ...
                if ( isset( $input ) && count( $input ) > 0 )
                {
                    ...
                    foreach ( $input as $name => $v )
                    {
                        ...
                        <dl>
                            <dt><?php echo $name; ?></dt>
                            <dd>
                                <pre><code><?php echo Debug::dump( $input[ $name ], 1000, 20 ); ?></code></pre>
                            </dd>
                        </dl>
            ...
            <div class="tabcontent-inner">
                <div class="box executed-sql">
                    <div class="sql-code"><?php
                        echo Debug::debugSqlQuery( $sqlerror, $data[ 'sql_line' ] );
                        ?>
                        <div class="col-sep"></div>
                    </div>

                    <?php
                    if ( isset( $sqlargs ) && count( $sqlargs ) > 0 )
                    {
                        ?>
                        ...
                        <div class="sql-args">
                            <h3>Arguments</h3>
                            <?php
                            foreach ( $sqlargs as $name => $v )
                            {
                                ?>
                                <dl>
                                    <dt><?php echo $name; ?></dt>
                                    <dd>
                                        <pre><code><?php echo Debug::dump( $sqlargs[ $name ], 1000, 20 ); ?></code></pre>
                                    </dd>
                                </dl>
