...
class GeoIP
{
     ...
     * @return void
     *
     * @throws BaseException if unable to open specified file or shared memory.
     */
    public function open( $filename, $flags = null )
    {
        ...
        if ( $this->flags & self::SHARED_MEMORY )
        {
            ...
            if ( $this->shmid === false )
            {
                ...
                $this->shmid = @shmop_open( self::SHM_KEY, "a", 0, 0 );
                if ( $this->shmid === false )
                { // should never be false as loadSharedMemory() will throw Exc if cannot create
                    throw new BaseException( "Unable to open shared memory at key: " . dechex( self::SHM_KEY ) );
                }
            }
        }
        else
        {
            ...
            if ( !$this->filehandle )
            {
                throw new BaseException( "Unable to open file: $filename" );
            }
     ...
     * @return void
     *
     * @throws BaseException     - if unable to read the db file.
     */
     ...
     *
     * @throws BaseException  - if IP address is invalid.
    ...
    protected function lookupCountryId( $addr )
    {
        ...
        if ( $ipnum === false )
        {
            throw new BaseException( "Invalid IP address: " . var_export( $addr, true ), self::ERR_INVALID_IP );
        }
        if ( $this->databaseType !== self::COUNTRY_EDITION )
        {
            throw new BaseException( "Invalid database type; lookupCountry*() methods expect Country database." );
        }
     ...
     *
     * @throws BaseException (see lookupCountryId())
     * @see lookupCountryId()
     */
     ...
     * @throws BaseException - if fseek() fails on the file or no results after traversing the database (indicating corrupt db).
     */
    protected function seekCountry( $ipnum )
    {
        ...
        for ( $depth = 31; $depth >= 0; --$depth )
        {
            ...
            else
            {
                ...
                {
                    throw new BaseException( "fseek failed" );
                }
        ...
        throw new BaseException( "Error traversing database - perhaps it is corrupt?" );
    }
    ...
    public function lookupOrg( $addr )
    {
        ...
        if ( $this->databaseType !== self::ORG_EDITION )
        {
            throw new BaseException( "Invalid database type; lookupOrg() method expects Org/ISP database.", self::ERR_DB_FORMAT );
        }
    ...
    public function lookupRegion( $addr )
    {
        ...
        {
            throw new BaseException( "Invalid database type; lookupRegion() method expects Region database.", self::ERR_DB_FORMAT );
        }
    ...
    public function lookupLocation( $addr )
    {
        ...
        {
            throw new BaseException( "Invalid database type; lookupLocation() method expects City database." );
        }
