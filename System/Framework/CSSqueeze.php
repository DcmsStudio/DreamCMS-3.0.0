...
class CSSqueeze
{
    ...
    public function squeeze( $css, $singleLine = true, $keepHack = true )
    {
        ...
        $css = !is_file( $this->configuration[ 'BasePath' ] . $css ) ? $css : file_get_contents( $this->configuration[ 'BasePath' ] . $css );
        $css = trim( $css );
        if ( '' === $css )
            return '';
    ...
    protected function getCSS( $css )
    {
        ...
        foreach ( array_keys( $captures[ 0 ] ) as $key )
        {
            ...
            else
            {
                ...
                array_key_exists( $groupKey, $groups ) || $groups[ $groupKey ] = array(
                    $media,
                    '{' );
    ...
    protected function mergeSelectors( $selectors, $blocks )
    {
        ...
        foreach ( $a as $k => $v )
        {
            ...
            while ( list($k2, $v2) = each( $selectors ) )
            {
                ...
                switch ( true )
                {
                    case (isset( $b[ $k ] )) : $b[ $k ] .= $blocks[ $k2 ];
                        break;
                    case (isset( $blocks[ $k2 ] )) : $b[ $k ] = $blocks[ $k2 ];
                        break;
                }
            }
        }
    ...
    protected function prepareComments( $css )
    {
        ...
        return preg_replace(
                array_keys( $pregReplace ), array_values( $pregReplace ), $css
        );
    }
    ...
    public function sorter( $block )
    {
        ...
        foreach ( $declarations as $declaration )
        {
            ...
            $a[ $property ] = (isset( $this->colorValues[ $property ] )) ? $this->getShortestColor( $value ) : $value;
        }
    ...
    protected function getShortestColor( $color )
    {
        ...
        return (isset( $this->shortColor[ $color ] )) ? $this->shortColor[ $color ] : $color;
    }
     ...
     * @internal param string $indent string indent
     *
