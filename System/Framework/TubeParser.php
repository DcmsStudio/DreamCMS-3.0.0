...
class TubeParser
{
    ...
    public function __construct()
    {
        ...
        include_once DATA_PATH . 'system/tube_parser_data.php';

        $this->_embed = $AutoEmbed_stubs;

        $AutoEmbed_stubs = null;
    }
    ...
    public function parseUrl( $url )
    {
        ...
        foreach ( $this->_embed as $stub )
        {
            if ( preg_match( '~' . $stub[ 'url-match' ] . '~ism', $url, $match ) )
            {
                $this->_stub = $stub;
    ...
    private function _parseLink( $url )
    {
        ...
        if ( preg_match( '~' . $this->_stub[ 'fetch-match' ] . '~imu', $url, $match ) )
        {
            $this->_media_id = $match;
            $this->_setDefaultParams();
            return true;
    ...
    {
        if ( !count($this->_object_attribs) || !count($this->_object_params) ) {
            return false;
        }
    ...
    private function _setDefaultParams()
    {
        ...
        if (!$flashvars) {
            unset($this->_object_params['flashvars']);
