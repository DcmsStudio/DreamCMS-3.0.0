...
class Config_Ini extends Config
{
     ...
     *      hostname = staging
     *
     * after calling $data = new Config_Ini($file, 'staging'); then
    ...
    public function __construct( $filename, $section = null, $options = false )
    {
        if ( empty( $filename ) )
        {
            throw new BaseException( 'Filename is not set' );
        }
        ...
        else
        {
            ...
            foreach ( $section as $sectionName )
            {
                ...
                {
                    throw new BaseException( "Section '$sectionName' cannot be found in $filename" );
                }
    ...
    protected function _parseIniFile( $filename )
    {
        ...
        {
            throw new BaseException( $this->_loadFileErrorStr );
        }
            ...
            switch ( count( $pieces ) )
            {
                ...
                case 2:
                    ...
                    $iniArray[ $thisSection ] = array_merge( array(
                        ';extends' => $extendedSection ), $data );
                    break;

                default:
                    throw new BaseException( "Section '$thisSection' may not extend multiple sections in $filename" );
            }
        }
    ...
    protected function _processSection( $iniArray, $section, $config = array() )
    {
        ...
        foreach ( $thisSection as $key => $value )
        {
            if ( strtolower( $key ) == ';extends' )
            {
                ...
                {
                    throw new BaseException( "Parent section '$section' cannot be found" );
                }
            }
            else
            {
    ...
    protected function _processKey( $config, $key, $value )
    {
        if ( strpos( $key, $this->_nestSeparator ) !== false )
        {
            ...
            if ( strlen( $pieces[ 0 ] ) && strlen( $pieces[ 1 ] ) )
            {
                ...
                {
                    throw new BaseException( "Cannot create sub-key for '{$pieces[ 0 ]}' as key already exists" );
                }
                $config[ $pieces[ 0 ] ] = $this->_processKey( $config[ $pieces[ 0 ] ], $pieces[ 1 ], $value );
            }
            else
            {
                throw new BaseException( "Invalid key '$key'" );
            }
        }
        else
        {
