...
class Request
{
    ...
    {
        $this->strUserAgent = 'Mozilla/5.0 (compatible; DreamCMS ' . VERSION . '; rv:1.0)';
        $this->strData = '';
        $this->strMethod = 'get';
    }
     ...
     * @return string
     */
    function _decodeChunked( $buffer, $lb )
    {
        // length := 0
        $length = 0;
        $new = '';
    ...
    protected function connect()
    {
        ...
        if ( $this->arrProxy[ 'proxyhost' ] )
        {
            ...
            if ( $this->arrUri[ 'scheme' ] === 'https' )
            {
                ...
                STREAM_CRYPTO_METHOD_SSLv2_CLIENT ) as $mode )
                    {
                        if ( $success = stream_socket_enable_crypto( $this->socket, true, $mode ) )
                        {
                            break;
                        }
                    }
    ...
    protected function compileHeaders()
    {
        ...
        {
            $headers = array(
                'Host' => 'Host: ' . ($this->isIP6( $this->arrUri[ 'host' ] ) ? '[' . $this->arrUri[ 'host' ] . ']' : $this->arrUri[ 'host' ]) . ((isset( $this->arrUri[ 'addport' ] ) && $this->arrUri[ 'addport' ]) ? ':' . $this->arrUri[ 'port' ] : '') );
        }
        ...
        do
        {
            ...
            switch ( $this->intCode )
            {
                ...
                case 303:
                    ...
                    {
                        // redirect..
                        if ( ($newurl = @parse_url( $this->arrResponseHeaders[ 'Location' ] )) !== false && $this->_redirects < $this->_maxRedirects )
                        {
                            if ( $newurl[ 'schema' ] )
                            {
                                $this->arrUri = $newurl;
