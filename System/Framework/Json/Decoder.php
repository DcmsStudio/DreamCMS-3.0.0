...
class DCMS_Json_Decoder
{
     ...
     * {@link Json::TYPE_ARRAY} and {@link Json::TYPE_OBJECT} for
     *                           valid values
     * @return \DCMS_Json_Decoder
     */
     ...
     *
     * Throws a DCMS_Json_Exception if the source string is null.
     *
     * @static
     * @access public
     * @param string $source String to be decoded
     * @param int $objectDecodeType How objects should be decoded; should be
     * either or {@link Json::TYPE_ARRAY} or
     * {@link Json::TYPE_OBJECT}; defaults to TYPE_ARRAY
     * @return mixed
    ...
    public static function decode( $source = null, $objectDecodeType = Json::TYPE_ARRAY )
    {
        if ( null === $source )
        {
            ...
            throw new BaseException( 'Must specify JSON encoded source for decoding' );
        }
        elseif ( !is_string( $source ) )
        {

            throw new BaseException( 'Can only decode JSON encoded strings' );
        }
     ...
     *
     * If Json_Encoder was used to encode the original object then
    ...
    protected function _decodeObject()
    {
        ...
        while ( $tok && $tok != self::RBRACE )
        {
            if ( $tok != self::DATUM || !is_string( $this->_tokenValue ) )
            {
                ...
                throw new BaseException( 'Missing key in object encoding: ' . $this->_source );
            }
            ...
            if ( $tok != self::COLON )
            {
                ...
                throw new BaseException( 'Missing ":" in object encoding: ' . $this->_source );
            }
            ...
            if ( $tok != self::COMMA )
            {
                ...
                throw new DCMS_Json_Exception( 'Missing "," in object encoding: ' . $this->_source );
            }
    ...
    protected function _decodeArray()
    {
        ...
        while ( $tok && $tok != self::RBRACKET )
        {
            ...
            if ( $tok != self::COMMA )
            {
                ...
                throw new BaseException( 'Missing "," in array encoding: ' . $this->_source );
            }
    ...
    protected function _getNextToken()
    {
                    ...
                    if ( $chr == '\\' )
                    {
                        ...
                        switch ( $chr )
                        {
                            ...
                            case '\'' :
                                ...
                                break;
                            default:
                                throw new Json_Exception( "Illegal escape "
                                . "sequence '" . $chr . "'" );
                        }
                    }
                    elseif ( $chr == '"' )
        ...
        if ( $chr == '-' || $chr == '.' || ($chr >= '0' && $chr <= '9') )
        {
            if ( preg_match( '/-?([0-9])*(\.[0-9]*)?((e|E)((-|\+)?)[0-9]+)?/s', $str, $matches, PREG_OFFSET_CAPTURE, $start ) && $matches[ 0 ][ 1 ] == $start )
            {
                ...
                if ( is_numeric( $datum ) )
                {
                    ...
                    {
                        throw new BaseException( "Octal notation not supported by JSON (value: $datum)" );
                    }
                    else
                    {
                        $val = (int)$datum ;
                ...
                {
                    throw new BaseException( "Illegal number format: $datum" );
                }
        ...
        else
        {
            ...
            throw new BaseException( 'Illegal Token' );
        }
